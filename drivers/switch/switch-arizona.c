...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 03-104 */
#include <linux/delay.h>
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 03-104 */
...
static void arizona_start_mic(struct arizona_extcon_info *info)
{
	...
	regmap_update_bits_check(arizona->regmap, ARIZONA_MIC_DETECT_1,
				 ...
				 &change);
	regmap_update_bits(arizona->regmap, ARIZONA_MIC_BIAS_CTRL_1, ARIZONA_MICB1_ENA, ARIZONA_MICB1_ENA);
	if (!change) {
...
static void arizona_identify_headphone(struct arizona_extcon_info *info)
{
	...
	if (arizona->pdata.hpdet_disable) {
		dev_dbg(arizona->dev, "Skipping HPDET\n");
		goto err;
	}
...
static void arizona_micd_detect(struct work_struct *work)
{
	...
	if (!(val & ARIZONA_MICD_STS)) {
		...
		if (!arizona->pdata.infinite_micd) {
			info->detecting = false;
		}
		goto handled;
	}
...
handled:
	if (info->detecting && !arizona->pdata.infinite_micd)
...
static irqreturn_t arizona_jackdet(int irq, void *data)
{
...
#ifdef CONFIG_SH_AUDIO_DRIVER  /* 03-104 */
#if (CONFIG_SH_AUDIO_DRIVER_MODEL_NUMBER == 103) || (CONFIG_SH_AUDIO_DRIVER_MODEL_NUMBER == 104) || (CONFIG_SH_AUDIO_DRIVER_MODEL_NUMBER == 202)
	if ((arizona->hw_rev == HW_REVISION_ES0) || (arizona->hw_rev == HW_REVISION_ES1)) {
		dev_info(arizona->dev, "call msleep()");
		msleep(500);
	}
#elif (CONFIG_SH_AUDIO_DRIVER_MODEL_NUMBER == 105)
	if (arizona->hw_rev == HW_REVISION_ES0) {
		dev_info(arizona->dev, "call msleep()");
		msleep(500);
	}
	...
	if (val == info->last_jackdet) {
		...
		if (cancelled_hp && !arizona->pdata.hpdet_disable)
			schedule_delayed_work(&info->hpdet_work,
					      msecs_to_jiffies(HPDET_DEBOUNCE));

		if (cancelled_mic && !arizona->pdata.infinite_micd)
			schedule_delayed_work(&info->micd_timeout_work,
					      msecs_to_jiffies(info->micd_timeout));
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 03-011 */
int arizona_headset_hp_state(void)
{
	if (h2w_sdev){
		return h2w_sdev->state;
	}

	return 0;
}
EXPORT_SYMBOL_GPL(arizona_headset_hp_state);

int arizona_headset_bu_state(void)
{
	return bu_state;
}
EXPORT_SYMBOL_GPL(arizona_headset_bu_state);
