...
s64 pm8xxx_cc_scale_uv_to_ma(s64 uv)
{
	...
	{
		result_mA = div_s64(uv * 1000LL, the_chip->r_sense_uohm);
	}
...
bail:
#ifdef CONFIG_BATTERY_SH
	rc = pm8xxx_writeb(the_chip->dev->parent, ADC_ARB_SECP_CNTRL, sec_cntrl);
	if (rc)
		pr_debug("error = %d writing ADC_ARB_SECP_CNTRL:0x%x,%d,0x%x\n", rc, (unsigned int)the_chip->dev->parent, ADC_ARB_SECP_CNTRL, (unsigned int)sec_cntrl);
