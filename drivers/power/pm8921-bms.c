...
static void readjust_fcc_table(void)
{
	...
	for (i = 0; i < the_chip->fcc_temp_lut->cols; i++) {
		...
		ratio = div_u64((u64)the_chip->fcc_temp_lut->y[i] * 1000, fcc);
...
int pm8921_bms_get_last_good_ocv(uint *result, uint *adc_code)
{
	...
	if (the_chip)
	{
		...
		rc = read_soc_params_raw(the_chip, &raw, batt_temp);
...
void pm_bms_masked_disable( void )
{
	...
	pm_bms_masked_write(the_chip, BMS_CONTROL,BMS_MODE_BIT, BMS_MODE_BIT);
