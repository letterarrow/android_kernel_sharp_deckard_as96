	...
	enum msm_camera_i2c_data_type data_type)
{
...
#else
	if (!client || !reg_tbl){
		pr_err(" client =%x reg_tbl =%x", (unsigned int)client, (unsigned int)reg_tbl );
		return rc;
	}
#endif
	...
	if ((client->addr_type != MSM_CAMERA_I2C_BYTE_ADDR
		...
		pr_err(" addr_type, data_type = %d  %d", client->addr_type, data_type );
	...
	for (i = 0; i < size; i++) {
		...
		if (reg_tbl->delay){
			S_I2C_DBG("i =%d, delay =%d", i, reg_tbl->delay );
			usleep_range(reg_tbl->delay, reg_tbl->delay + 1000);
		}
#endif
		reg_tbl++;
