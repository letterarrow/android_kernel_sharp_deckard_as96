...
void msm_sensor_adjust_frame_lines1(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->sensor_exp_gain_info) {
		...
		fl_lines = s_ctrl->msm_sensor_reg->output_settings[s_ctrl->curr_res].frame_length_lines;
		fl_lines = (fl_lines * s_ctrl->fps_divider) / Q10;

		if ( cur_line > (fl_lines - s_ctrl->sensor_exp_gain_info->vert_offset) ){
			fl_lines = cur_line + s_ctrl->sensor_exp_gain_info->vert_offset;
			CDBG("%s update lines=%d\n", __func__, fl_lines);
			msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				s_ctrl->sensor_output_reg_addr->
				frame_length_lines,
				fl_lines,
