...
static int panic_time = 5;
module_param_named(panic_time, panic_time, int, S_IRUGO | S_IWUSR | S_IWGRP);
#endif

static int is_hotstandby = 0;
module_param_named(hotstandby, is_hotstandby, int, S_IRUGO | S_IWUSR | S_IWGRP);
...
#ifdef CONFIG_PMIC8XXX_SCPWRKEY
	if (sh_boot_get_bootmode() != SH_BOOT_O_C && !is_hotstandby) {
