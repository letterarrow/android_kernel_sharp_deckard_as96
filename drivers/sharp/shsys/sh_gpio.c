...
static int sh_gpio_ioctl_write(unsigned long arg)
{
	 ...
	 * Write GPIO Value
	 */
	config = (outval << 9) | (drvstr << 6) | (func << 2) | pull;

	if (func == GPIOMUX_FUNC_GPIO) {
		config |= dir > GPIOMUX_IN ? BIT(9) : 0;
...
static int sh_gpio_ioctl_write_pmic_gpio(unsigned long arg)
{
    ...
    if (copy_from_user(&gpio_req, (void __user *)arg, sizeof(struct sh_pm_gpio)) != 0) {
        printk("sh_gpio_ioctl_write_pmic_gpio: copy_from_user() error (%d line)\n", __LINE__);
        return -EINVAL;
    }


	config.direction	= gpio_req.direction; 
	config.output_buffer= gpio_req.output_buffer;
	config.output_value	= gpio_req.output_value;
	config.pull			= gpio_req.pull;
	config.vin_sel		= gpio_req.vin_sel;
	config.out_strength	= gpio_req.out_strength;
	config.function		= gpio_req.function;
	config.inv_int_pol	= gpio_req.inv_int_pol;
	config.disable_pin	= gpio_req.disable_pin;

	rc = gpio_request( gpio_req.gpio , NULL );
	if ( rc ) {
        printk("sh_gpio_ioctl_write_pmic_gpio: gpio_request() error (%d line)\n", __LINE__);
		return rc ;
	}

	rc = pm8xxx_gpio_config(gpio_req.gpio, &config);
	if (rc) {
        printk("sh_gpio_ioctl_write_pmic_gpio: pm8xxx_gpio_config() error (%d line)\n", __LINE__);
	}
	gpio_free(gpio_req.gpio);

    return rc;
}
static int sh_gpio_ioctl_write_pmic_mpp(unsigned long arg)
{
    int rc = 0;
    struct sh_pm8xxx_mpp_config_data 	gpio_req;
    struct pm8xxx_mpp_config_data 	  	config;

    if (arg == 0) {
        printk("sh_gpio_ioctl_write_pmic_mpp: paramter error (%d line)\n", __LINE__);
        return -EINVAL;
    }

    if (copy_from_user(&gpio_req, (void __user *)arg, sizeof(struct sh_pm8xxx_mpp_config_data)) != 0) {
        printk("sh_gpio_ioctl_write_pmic_mpp: copy_from_user() error (%d line)\n", __LINE__);
        return -EINVAL;
    }

	config.type		= gpio_req.type;
	config.level	= gpio_req.level;
	config.control	= gpio_req.control;

	rc = gpio_request( gpio_req.gpio , NULL );
	if ( rc ) {
        printk("sh_gpio_ioctl_write_pmic_mpp: gpio_request() error (%d line)\n", __LINE__);
		return rc ;
	}

	rc = pm8xxx_mpp_config(gpio_req.gpio, &config);
	if (rc) {
        printk("sh_gpio_ioctl_write_pmic_mpp: pm8xxx_mpp_config() error (%d line)\n", __LINE__);
	}

	gpio_set_value(gpio_req.gpio , gpio_req.output_value);
	gpio_free(gpio_req.gpio);
...
static long sh_gpio_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
   ...
   case SH_GPIO_WRITE_PMIC_GPIO:
        ...
        break;
   case SH_GPIO_WRITE_PMIC_MPP:
        rc = sh_gpio_ioctl_write_pmic_mpp(arg);
        break;
    default:
