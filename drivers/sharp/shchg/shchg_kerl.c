...
#define SHCHG_DRV_IOCTL_CMD_GET_MHL_CABLE_STATUS _IOR( SHCHG_IOC_MAGIC,  31, int)
...
shchg_result_t shchg_api_get_mhl_cable_status( int* mhl_cable_status );
...
static shchg_result_t shchg_pm_get_charger_vmaxsel( int* mv_p )
{
	...
	if(mv_p != NULL)
	{
		rc = pm8921_charger_vddmax_get(mv_p);
		if (rc)
		{
			result = SHBATT_RESULT_FAIL;
			SHCHG_ERROR("error pm8921_charger_vddmax_get rc = %d\n", rc);
		}
	}
	SHCHG_TRACE("[E] %s \n",__FUNCTION__);
	return result;
