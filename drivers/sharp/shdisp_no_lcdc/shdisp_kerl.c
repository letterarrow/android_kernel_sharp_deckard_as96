/* drivers/sharp/shdisp_no_lcdc/shdisp_kerl.c  (Display Driver)
 *
...
static void shdisp_set_irq_port(void);
...
static int shdisp_SQE_main_disp_ext_clk_on(void);
static int shdisp_SQE_main_disp_ext_clk_off(void);
...
int shdisp_api_main_disp_init_1st(void)
{
    ...
    SHDISP_PERFORMANCE("RESUME PANEL DISP-INIT 0010 START\n");
    ...
    SHDISP_PERFORMANCE("RESUME PANEL DISP-INIT 0010 END\n");
...
int shdisp_api_main_disp_init_2nd(void)
{
    ...
    SHDISP_PERFORMANCE("RESUME PANEL DISP-INIT 0020 START\n");
    ...
    SHDISP_PERFORMANCE("RESUME PANEL DISP-INIT 0020 END\n");
...
int shdisp_api_main_disp_on(void)
{
    ...
    SHDISP_PERFORMANCE("RESUME PANEL DISP-ON 0010 START\n");
    ...
    SHDISP_PERFORMANCE("RESUME PANEL DISP-ON 0010 END\n");
...
int shdisp_api_main_disp_off(void)
{
    ...
    SHDISP_PERFORMANCE("SUSPEND PANEL DISP-OFF 0010 START\n");
    ...
    SHDISP_PERFORMANCE("SUSPEND PANEL DISP-OFF 0010 END\n");
...
/* ------------------------------------------------------------------------- */
/* shdisp_api_main_disp_ext_clk_on                                           */
/* ------------------------------------------------------------------------- */
...
int shdisp_api_main_disp_ext_clk_on(void)
{
    ...
    SHDISP_TRACE("shdisp_api_main_disp_ext_clk_on\n");
    
    shdisp_semaphore_start();
    
    ret = shdisp_SQE_main_disp_ext_clk_on();
    
    shdisp_semaphore_end(__func__);
    
    if (ret != SHDISP_RESULT_SUCCESS) {
        SHDISP_ERR("<RESULT_FAILURE> shdisp_SQE_main_disp_ext_clk_on.\n");
        return SHDISP_RESULT_FAILURE;
    }
...
/* ------------------------------------------------------------------------- */
/* shdisp_api_main_disp_ext_clk_off                                          */
/* ------------------------------------------------------------------------- */
...
int shdisp_api_main_disp_ext_clk_off(void)
{
    ...
    SHDISP_TRACE("shdisp_api_main_disp_ext_clk_off\n");
    
    shdisp_semaphore_start();
    
    ret = shdisp_SQE_main_disp_ext_clk_off();
    
    shdisp_semaphore_end(__func__);
    
    if (ret != SHDISP_RESULT_SUCCESS) {
        SHDISP_ERR("<RESULT_FAILURE> shdisp_SQE_main_disp_ext_clk_off.\n");
        return SHDISP_RESULT_FAILURE;
    }
...
/* ------------------------------------------------------------------------- */
/* shdisp_api_get_bdic_reset_port                                            */
/* ------------------------------------------------------------------------- */

int shdisp_api_get_bdic_reset_port(void)
{
     return shdisp_kerl_ctx.bdic_reset_port;
}
...
static void shdisp_init_context(void)
{
    ...
    shdisp_kerl_ctx.bdic_reset_port             = shdisp_boot_ctx.bdic_reset_port;
...
#if defined(CONFIG_SHDISP_PANEL_KITE)
            if (shdisp_kite_API_create()->check_upper_unit() != SHDISP_RESULT_SUCCESS) {
                local_boot_context->upper_unit_is_connected = SHDISP_UPPER_UNIT_IS_NOT_CONNECTED;
            }
#endif
        } else {
#if defined(CONFIG_SHDISP_PANEL_RYOMA)
            if (shdisp_ryoma_API_create()->check_upper_unit() != SHDISP_RESULT_SUCCESS) {
                local_boot_context->upper_unit_is_connected = SHDISP_UPPER_UNIT_IS_NOT_CONNECTED;
            }
#endif
        }
...
static void shdisp_context_initialize(int mode)
{
    if (mode == 0) {
        ...
        shdisp_boot_ctx.bdic_reset_port             = SHDISP_GPIO_NUM_BL_RST_NEW;
    }
    else {
...
#if defined(CONFIG_MACH_LYNX_DL20)
    if (  (shdisp_boot_ctx.hw_revision == SHDISP_HW_REV_ES0)
       || (shdisp_boot_ctx.hw_revision == SHDISP_HW_REV_ES05)
       || (shdisp_boot_ctx.hw_revision == SHDISP_HW_REV_ES1)
       || (shdisp_boot_ctx.hw_revision == SHDISP_HW_REV_ES15)
       || (shdisp_boot_ctx.hw_revision == SHDISP_HW_REV_PP1)
       || (shdisp_boot_ctx.hw_revision == SHDISP_HW_REV_PP15)
       ) {
        irq_port_ver = 0;
    }
    else{
...
#endif
    shdisp_SYS_set_irq_port( irq_port_ver );
}
...
static long shdisp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case SHDISP_IOCTL_PWM_CONFIG:
        SHDISP_PERFORMANCE("RESUME PANEL PWM-CONFIG 0010 START\n");
        ret = shdisp_ioctl_pwm_config(argp);
        SHDISP_PERFORMANCE("RESUME PANEL PWM-CONFIG 0010 END\n");
        break;
    case SHDISP_IOCTL_PWM_STOP:
        SHDISP_PERFORMANCE("SUSPEND PANEL PWM-STOP 0010 START\n");
        ret = shdisp_ioctl_pwm_stop();
        SHDISP_PERFORMANCE("SUSPEND PANEL PWM-STOP 0010 END\n");
        break;
...
/* ------------------------------------------------------------------------- */
/* shdisp_SQE_main_disp_ext_clk_on                                           */
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
/* shdisp_SQE_main_disp_ext_clk_off                                          */
/* ------------------------------------------------------------------------- */
...
#ifdef SHDISP_BL68Y6
    SHDISP_ERR("<OTHER> BKL PWM ENABLE : not supported for BL68Y6.\n");
#else /* SHDISP_BL68Y6 */
...
#ifdef SHDISP_BL68Y6
    SHDISP_ERR("<OTHER> BKL PWM DISABLE : not supported for BL68Y6.\n");
#else /* SHDISP_BL68Y6 */
...
{
#ifdef SHDISP_BL68Y6
    SHDISP_ERR("<OTHER> shdisp_SQE_pwm_enable : not supported for BL68Y6.\n");
#else /* SHDISP_BL68Y6 */
    shdisp_bdic_API_LCD_BKL_pwm_enable();
#endif /* SHDISP_BL68Y6 */
...
{
#ifdef SHDISP_BL68Y6
    SHDISP_ERR("<OTHER> shdisp_SQE_pwm_disable : not supported for BL68Y6.\n");
#else /* SHDISP_BL68Y6 */
    shdisp_bdic_API_LCD_BKL_pwm_disable();
#endif /* SHDISP_BL68Y6 */
...
static int shdisp_proc_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
#define SHDISP_LEN_CATEGORY (1)
#define SHDISP_LEN_ID       (2)
#define SHDISP_LEN_HEADER   (SHDISP_LEN_CATEGORY+SHDISP_LEN_ID)
...
#define SHDISP_PARAM_MAX    (4)
    ...
    char kbuf[SHDISP_LEN_HEADER + SHDISP_PARAM_MAX * SHDISP_LEN_PARAM];
    int i;
    int ret = 0;
    ...
    len--;
    /* Check length */
    if (len < (SHDISP_LEN_HEADER)) {
        return count;
    }
    if (len > (SHDISP_LEN_HEADER + SHDISP_PARAM_MAX * SHDISP_LEN_PARAM)) {
       len = SHDISP_LEN_HEADER + SHDISP_PARAM_MAX * SHDISP_LEN_PARAM;
    }
    ...
    memset(&shdisp_pfs, 0x00, sizeof(struct shdisp_procfs));

    /* Get Category */
    memcpy(buf, &(kbuf[0]), SHDISP_LEN_CATEGORY);
    buf[SHDISP_LEN_CATEGORY] = '\0';
    shdisp_pfs.category = simple_strtol(buf, NULL, 10);

    /* Get FunctionID */
    memcpy(buf, &(kbuf[SHDISP_LEN_CATEGORY]), SHDISP_LEN_ID);
    buf[SHDISP_LEN_ID] = '\0';
    shdisp_pfs.id = simple_strtol(buf, NULL, 10);

    /* Get Parameters */
    for (i = 0; (i + 1) * SHDISP_LEN_PARAM <= (len - SHDISP_LEN_HEADER); i++) {
        memcpy(buf, &(kbuf[SHDISP_LEN_HEADER + i * SHDISP_LEN_PARAM]), SHDISP_LEN_PARAM);
        buf[SHDISP_LEN_PARAM] = '\0';
        shdisp_pfs.par[i] = simple_strtol(buf, NULL, 16);
    }

    printk("[SHDISP] shdisp_proc_write(%d, %d, 0x%04x, 0x%04x, 0x%04x, 0x%04x)\n", shdisp_pfs.category, shdisp_pfs.id, shdisp_pfs.par[0], shdisp_pfs.par[1], shdisp_pfs.par[2], shdisp_pfs.par[3]);

    switch (shdisp_pfs.category) {
    case SHDISP_DEBUG_FUNCTION:
    ...
    case SHDISP_DEBUG_KERNEL:
        switch (shdisp_pfs.id) {
        case SHDISP_DEBUG_LCD_DISP_CTL:
            printk("[SHDISP] LCD_DISP_CTL (mode : %d)\n", shdisp_pfs.par[0]);
            switch (shdisp_pfs.par[0]) {
            case 0:
                printk("[SHDISP] lcd power off\n");
                ret = shdisp_api_main_lcd_power_off();
                break;
            case 1:
                printk("[SHDISP] lcd power on\n");
                ret = shdisp_api_main_lcd_power_on();
                break;
            case 2:
                printk("[SHDISP] lcd displayr off\n");
                ret = shdisp_api_main_disp_off();
                break;
            case 3:
                printk("[SHDISP] lcd displayr on\n");
                ret = shdisp_api_main_disp_on();
                break;
            default:
                printk("[SHDISP] lcd display_ctl_param_error\n");
                break;
            }
            break;
        ...
        case SHDISP_DEBUG_XEN_SENSOR_BIT_CLEAR:
            ...
            shdisp_semaphore_end(__func__);
            break;
        case SHDISP_DEBUG_CHECK_BLACKSCREEN_TIMEOUT:
            i = shdisp_pfs.par[0];
