/* drivers/sharp/shdisp_no_lcdc/shdisp_kite.c  (Display Driver)
 *
...
static int shdisp_kite_API_diag_get_flicker_low_param(unsigned short *alpha);
...
static struct shdisp_panel_operations shdisp_kite_fops = {
    ...
    shdisp_kite_API_diag_get_flicker_low_param,
};
...
static void shdisp_kite_API_set_param(struct shdisp_panel_param_str *param_str)
{
    ...
    kite_param_str.vcom_alpha_low = param_str->vcom_alpha_low;
...
#define SHDISP_GPIO_NUM_UPPER_UNIT  28          /* TEST_MODE */
    ...
    pr_info("[SHDISP]check_upper_unit val=%d\n", val);
    if(val) {
