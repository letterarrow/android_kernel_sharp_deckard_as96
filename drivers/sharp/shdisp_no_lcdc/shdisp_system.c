/* drivers/sharp/shdisp_no_lcdc/shdisp_system.c  (Display Driver)
 *
...
//#define SHDISP_INT_IRQ MSM_GPIO_TO_INT(59)
#define SHDISP_INT_IRQ_V0 MSM_GPIO_TO_INT(59)
#define SHDISP_INT_IRQ_V1 MSM_GPIO_TO_INT(36)
...
static unsigned int shdisp_int_irq_port = SHDISP_INT_IRQ_V0;
...
void shdisp_SYS_Host_gpio_init(void)
{
    ...
    shdisp_host_gpio_request(shdisp_api_get_bdic_reset_port());
...
{
    shdisp_host_gpio_free(shdisp_api_get_bdic_reset_port());
...
int  shdisp_SYS_set_Host_gpio(int num, int value)
{
    ...
    if (  (num == shdisp_api_get_bdic_reset_port())
       || (num == SHDISP_GPIO_NUM_SPI_CS)
       || (num == SHDISP_GPIO_NUM_LCD_SCS_N)
...
#ifdef CONFIG_SHLCDC_LED_BD2802GU
    else if (num == SHDISP_GPIO_NUM_LEDC_RST_N) {
        gpio_set_value(num, value);
        return SHDISP_RESULT_SUCCESS;
    }
#endif
...
void shdisp_SYS_set_irq_port( int irq_port_ver )
{
    if( irq_port_ver == 1 ){
        shdisp_int_irq_port = SHDISP_INT_IRQ_V1;
    }
    else{
        shdisp_int_irq_port = SHDISP_INT_IRQ_V0;
    }
}
...
int  shdisp_SYS_set_irq( int enable )
{
    ...
    if( enable == SHDISP_IRQ_ENABLE ){
        if( shdisp_int_irq_port != SHDISP_INT_IRQ_V0 ){
...
static int shdisp_host_gpio_free(int num)
{
    ...
    if (  (num == shdisp_api_get_bdic_reset_port())
#ifdef CONFIG_SHLCDC_LED_BD2802GU
       || (num == SHDISP_GPIO_NUM_LEDC_RST_N)
#endif
       || (num == SHDISP_GPIO_NUM_CLK_SEL)
       ) {
        gpio_free(num);
