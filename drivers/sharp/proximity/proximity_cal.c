...
int PROX_recovery_end_func(void)
{
    ...
    if (atomic_read(&updata_flag) == 1) {
//  if(work_pending(&prox_updata_val_work)) {
//      cancel_work_sync(&prox_updata_val_work);
//  }
...
static int PROX_ConfigGPIO(drv_data *poProximityRec)
{
    ...
    poProximityRec->irq = MSM_GPIO_TO_INT(poProximityRec->irq_gpio);
