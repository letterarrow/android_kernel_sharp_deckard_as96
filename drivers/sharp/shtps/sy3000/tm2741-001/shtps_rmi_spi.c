/* drivers/sharp/shtps/sy3000/tm2741-001/shtps_rmi_spi.c
 *
...
static int shtps_init_param(struct shtps_rmi_spi *ts)
{
	...
	#else
		shtps_rmi_write(ts, 0xFF, 0x02);
		shtps_rmi_write(ts, ts->map.fn51.ctrlBase, 0x00);
		shtps_rmi_write(ts, 0xFF, 0x00);
...
static void shtps_read_touchevent(struct shtps_rmi_spi *ts, int state)
{
	...
	#if defined(SHTPS_MAXFINGER_5_ENABLE) && !defined(SHTPS_SPI_BLOCKACCESS_ENABLE)
