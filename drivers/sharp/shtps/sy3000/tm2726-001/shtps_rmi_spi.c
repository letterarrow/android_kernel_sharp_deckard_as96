/* drivers/sharp/shtps/sy3000/tm2726-001/shtps_rmi_spi.c
 *
 ...
 */
	...
	#define SHTPS_REZERO_AFTER_PHYSICAL_KEY_TOUCHUP_ENABLE
#endif /* SHTPS_PHYSICAL_KEY_ENABLE */

/* -----------------------------------------------------------------------------------
 */
#if defined( CONFIG_SHTPS_SY3000_TM2726_001 )
	#include "shtps_fw_tm2726-001.h"
...
module_param(hover_report_info_calc_type, int, S_IRUGO | S_IWUSR);
		...
		static int SHTPS_LPWG_NORMAL_0D_THRESHOLD = 		0x51;
		static int SHTPS_LPWG_QOS_LATENCY_DEF_VALUE = 		SHTPS_QOS_LATENCY_DEF_VALUE;
		static int SHTPS_LPWG_SWIPE_DIST_THRESHOLD = 		700;
		...
		#define SHTPS_LPWG_NORMAL_0D_THRESHOLD	 			0x51
		#define SHTPS_LPWG_QOS_LATENCY_DEF_VALUE	 		SHTPS_QOS_LATENCY_DEF_VALUE
		#define SHTPS_LPWG_SWIPE_DIST_THRESHOLD	 			700
	#endif /* SHTPS_DEBUG_VARIABLE_DEFINES ) */
...
static void shtps_get_lpwg_def_settings(struct shtps_rmi_spi *ts)
{
	...
	shtps_rmi_read_packet(ts, ts->map.fn12.ctrl.num[8].addr, data, sizeof(data));

	ts->lpwg.clip[0] = data[10];
	ts->lpwg.clip[1] = data[11];
}
...
static void shtps_set_lpwg_mode_on(struct shtps_rmi_spi *ts)
{
	...
	{
		u8 offset = F1A_QUERY_HASGENERALCONTROL(ts->map.fn1A.query.data) +
					F1A_QUERY_HASINTERRUPTENABLE(ts->map.fn1A.query.data) +
					F1A_QUERY_HASMULTIBUTTONSELECT(ts->map.fn1A.query.data) + 
					( F1A_QUERY_HASTXRXMAPPING(ts->map.fn1A.query.data) * ((F1A_QUERY_MAX_BUTTONCOUNT(ts->map.fn1A.query.data) + 1) * 2) );

		for(i = 0; i < SHTPS_PHYSICAL_KEY_NUM; i++){
			shtps_rmi_write(ts, ts->map.fn1A.ctrlBase + offset + i, SHTPS_LPWG_DOZE_0D_THRESHOLD);
		}
	}
	shtps_rmi_write(ts, ts->map.fn12.ctrl.num[27].addr, SHTPS_LPWG_DOZE_CTL);
#if 0
	...
	shtps_rmi_read_packet(ts, ts->map.fn12.ctrl.num[8].addr, data, sizeof(data));
	data[10] = SHTPS_LPWG_TX_CLIP_LO;
	data[11] = SHTPS_LPWG_TX_CLIP_HI;
	shtps_rmi_write_packet(ts, ts->map.fn12.ctrl.num[8].addr, data, sizeof(data));
...
static void shtps_set_lpwg_mode_off(struct shtps_rmi_spi *ts)
{
	...
	shtps_rmi_write(ts, ts->map.fn12.ctrl.num[27].addr, 0x00);
	if(ts->map.fn1A.enable)
	{
		int i;
		u8 offset = F1A_QUERY_HASGENERALCONTROL(ts->map.fn1A.query.data) +
		...
		for(i = 0; i < SHTPS_PHYSICAL_KEY_NUM; i++){
			shtps_rmi_write(ts, ts->map.fn1A.ctrlBase + offset + i, SHTPS_LPWG_NORMAL_0D_THRESHOLD);
		}
	}
	...
	shtps_rmi_read_packet(ts, ts->map.fn12.ctrl.num[8].addr, data, sizeof(data));
	data[10] = ts->lpwg.clip[0];
	data[11] = ts->lpwg.clip[1];
	shtps_rmi_write_packet(ts, ts->map.fn12.ctrl.num[8].addr, data, sizeof(data));
	...
	if(ts->map.fn1A.enable){
		...
		shtps_set_hover_detect_init(ts);
			...
			if(ts->map.fn1A.enable)
			{
				...
				for(i = 0; i < SHTPS_PHYSICAL_KEY_NUM; i++){
					rc = shtps_rmi_write(ts, ts->map.fn1A.ctrlBase + offset + i, SHTPS_LPWG_NORMAL_0D_THRESHOLD);
					SPI_ERR_CHECK(rc, err_exit);
				}
			}
		#endif /* #if defined( SHTPS_PHYSICAL_KEY_ENABLE ) */
...
static void shtps_set_touch_info(struct shtps_rmi_spi *ts, u8 *buf, struct shtps_touch_info *info)
{
	...
	for(i = 0; i < fingerMax; i++){
		...
		if(info->fingers[i].state != SHTPS_TOUCH_STATE_NO_TOUCH){
			SHTPS_LOG_EVENT(
				printk(KERN_DEBUG "[shtps]ObjectTypeAndStatus[%d] = %d(%s)\n",
					i,
					info->fingers[i].state,
					info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER ? "Finger" :
					info->fingers[i].state == SHTPS_TOUCH_STATE_PEN ? "Pen" :
					info->fingers[i].state == SHTPS_TOUCH_STATE_HOVER ? "Hover" :
					info->fingers[i].state == SHTPS_TOUCH_STATE_PALM ? "Palm" :
					info->fingers[i].state == SHTPS_TOUCH_STATE_UNKNOWN ? "Unknown" : "Other"
				);
			);
				...
				(((ts->hover_ignore_touch_info >> i) & 0x01) == 0) )
			{
				if( (1 < SHTPS_HOVER_HIST_COUNT_MAX) && (SHTPS_HOVER_HIST_COUNT_MAX < SHTPS_HOVER_HIST_MAX) )
				{
					...
					if( ts->hover_hist_count < (SHTPS_HOVER_HIST_COUNT_MAX - 1) ){
						...
						for(i = 1; i < SHTPS_HOVER_HIST_COUNT_MAX; i++){
							ts->hover_hist[i - 1] = ts->hover_hist[i];
						}
						...
						if(hover_debug_log_enable & 0x02){
							SHTPS_LOG_DBG_PRINT("hover hist update <%d>(%d, %d)(time : %lu)\n", i, x, y, ts->hover_hist[SHTPS_HOVER_HIST_COUNT_MAX - 1].time);
						}
					}
				}
			}
	...
	if(!ts->absorption_hold_enable){
		...
		for (i = 0; i < fingerMax; i++) {
			   ...
			   (info->fingers[i].state != SHTPS_TOUCH_STATE_NO_TOUCH && i == ts->absorption_hold_tu_finger))
			{
				...
				ts->absorption_hold_enable = 0;
				ts->absorption_hold_tu_finger = -1;
...
static void shtps_absorption_hold_delayed_work_function(struct work_struct *work)
{
	...
	mutex_lock(&shtps_ctrl_lock);
	if(ts->absorption_hold_enable && ts->absorption_hold_tu_finger >= 0){
		ts->absorption_hold_enable = 0;
		ts->absorption_hold_tu_finger = -1;
...
#if defined ( SHTPS_REZERO_AFTER_PHYSICAL_KEY_TOUCHUP_ENABLE )
	if(state == 0){
		int i;
		u8 fingerMax = shtps_get_fingermax(ts);
...
#endif /* SHTPS_REZERO_AFTER_PHYSICAL_KEY_TOUCHUP_ENABLE */
}
