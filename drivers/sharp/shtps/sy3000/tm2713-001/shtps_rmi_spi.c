/* drivers/sharp/shtps/sy3000/tm2713-001/shtps_rmi_spi.c
 *
...
#if defined(SHTPS_LPWG_MODE_ENABLE) || defined(SHTPS_LOW_POWER_MODE_ENABLE)
	#define SHTPS_DOZE_WAKEUP_THRESHOLD_CTRL_ENABLE
#endif /* SHTPS_LPWG_MODE_ENABLE || SHTPS_LOW_POWER_MODE_ENABLE */

/* -----------------------------------------------------------------------------------
 */
#if defined( CONFIG_SHTPS_SY3000_TM2713_001 )
	#include "shtps_fw_tm2713-001.h"
...
#define SHTPS_FINGER_WIDTH_FUZZ					0
...
#define SHTPS_QOS_LATENCY_DEF_VALUE	 			1999
...
#define SHTPS_TOUCH_CANCEL_COORDINATES_Y		9999
	...
	#define SHTPS_VEILVIEW_PATTERN				SHTPS_VEILVIEW_PATTERN_MONOCHROME_1H
		...
		static int SHTPS_HOVER_TEMPORAL_FILTER = 	0x04;

		#if defined( SHTPS_MODULE_PARAM_ENABLE )
			module_param(SHTPS_HOVER_THRESH_FOR_ADC, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_HOVER_TEMPORAL_FILTER, int, S_IRUGO | S_IWUSR);
		#endif /* SHTPS_MODULE_PARAM_ENABLE */

	#else
		#define SHTPS_HOVER_THRESH_FOR_ADC			0x2E
		#define SHTPS_HOVER_TEMPORAL_FILTER			0x04
		...
		static int SHTPS_EDGE_HOVER_FAIL_RANGE_Y =			0;
		...
		#define SHTPS_EDGE_HOVER_FAIL_RANGE_Y			0
...
module_param(hover_report_info_calc_type, int, S_IRUGO | S_IWUSR);
	...
	#if defined( SHTPS_DEBUG_VARIABLE_DEFINES )
		static int SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A =	20;
		static int SHTPS_EDGE_POS_ADJUST_AREA_SIZE_B =	40;
		static int SHTPS_EDGE_POS_ADJUST_AREA_SIZE_C =	60;
		static int SHTPS_EDGE_POS_ADJUST_VOLUME_S =		20;

		#if defined( SHTPS_MODULE_PARAM_ENABLE )
			module_param(SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_EDGE_POS_ADJUST_AREA_SIZE_B, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_EDGE_POS_ADJUST_AREA_SIZE_C, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_EDGE_POS_ADJUST_VOLUME_S, int, S_IRUGO | S_IWUSR);
		#endif /* SHTPS_MODULE_PARAM_ENABLE */

	#else
		#define SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A		20
		#define SHTPS_EDGE_POS_ADJUST_AREA_SIZE_B		40
		#define SHTPS_EDGE_POS_ADJUST_AREA_SIZE_C		60
		#define SHTPS_EDGE_POS_ADJUST_VOLUME_S			20
	#endif /* SHTPS_DEBUG_VARIABLE_DEFINES */
	...
	#else
		#define SHTPS_MULTI_HOVER_EDGEFAIL_RANGE_X	100
	...
	#define SHTPS_LPWG_REPORT_WG_ONLY						0x02
		...
		static int SHTPS_LPWG_DOZE_WAKEUP_THRESHOLD_DEF = 	0x2E;
		...
		static int SHTPS_LPWG_SLEEP_IN_WAIT_MS =			SHTPS_SLEEP_IN_WAIT_MS;
		static int SHTPS_LPWG_DOZE_IN_WAIT_MS =				68;
		...
		static int SHTPS_LPWG_MIN_NOTIFY_CANCEL_INTERVAL = 	800;
			...
			module_param(SHTPS_LPWG_SLEEP_IN_WAIT_MS, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_LPWG_DOZE_IN_WAIT_MS, int, S_IRUGO | S_IWUSR);
		...
		#define SHTPS_LPWG_DOZE_WAKEUP_THRESHOLD_DEF		0x2E
		...
		#define SHTPS_LPWG_SLEEP_IN_WAIT_MS	 				SHTPS_SLEEP_IN_WAIT_MS
		#define SHTPS_LPWG_DOZE_IN_WAIT_MS	 				68
...
#if defined(SHTPS_DOZE_WAKEUP_THRESHOLD_CTRL_ENABLE)
	#if defined( SHTPS_DEBUG_VARIABLE_DEFINES )
		static int SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_ECO  = 0x1E;
		static int SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_DTV  = 0x2E;
		static int SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_DEF  = 0x1E;

		#if defined( SHTPS_MODULE_PARAM_ENABLE )
			module_param(SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_ECO, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_DTV, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_DEF, int, S_IRUGO | S_IWUSR);
		#endif /* SHTPS_MODULE_PARAM_ENABLE */
	#else
		#define SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_ECO		0x1E
		#define SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_DTV		0x2E
		#define SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_DEF		0x1E
	#endif /* SHTPS_DEBUG_VARIABLE_DEFINES ) */
#endif /* SHTPS_DOZE_WAKEUP_THRESHOLD_CTRL_ENABLE */
	...
	#if defined( SHTPS_DEBUG_VARIABLE_DEFINES )
		static int	SHTPS_SHIFT_EDGE_INWARD             = 1;
		static int	SHTPS_SHIFT_EDGE_INWARD_OFFSET      = 10;

		#if defined( SHTPS_MODULE_PARAM_ENABLE )
			module_param(SHTPS_SHIFT_EDGE_INWARD, int, S_IRUGO | S_IWUSR);
			module_param(SHTPS_SHIFT_EDGE_INWARD_OFFSET, int, S_IRUGO | S_IWUSR);
		#endif /* SHTPS_MODULE_PARAM_ENABLE */
	#else
		#define SHTPS_SHIFT_EDGE_INWARD                 1
		#define SHTPS_SHIFT_EDGE_INWARD_OFFSET          10
	#endif /* SHTPS_DEBUG_VARIABLE_DEFINES ) */
...
enum{
	SHTPS_PHYSICAL_KEY_BACK = 0,
	SHTPS_PHYSICAL_KEY_HOME,
	SHTPS_PHYSICAL_KEY_MENU,
...
enum{
	...
	SHTPS_LPMODE_CONTINUOUS_REQ_ON_FOR_DTV	= 0x04,
	SHTPS_LPMODE_ON						= ( SHTPS_LPMODE_NON_CONTINUOUS_REQ_ON
											| SHTPS_LPMODE_CONTINUOUS_REQ_ON
											| SHTPS_LPMODE_CONTINUOUS_REQ_ON_FOR_DTV),
};
#endif /* #if defined( SHTPS_LOW_POWER_MODE_ENABLE ) */
	...
	if(on){
		...
		shtps_delayed_rezero_cancel(ts);
			...
			if (ts->lpwg.lpwg_switch){
				...
				val |= 0x01;
				shtps_rmi_write(ts, ts->map.fn01.ctrlBase, val);
				SHTPS_LOG_DBG_PRINT("[LPWG] device set sleep [0x%02X]", val);
				if(SHTPS_LPWG_SLEEP_IN_WAIT_MS > 0){
					mdelay(SHTPS_LPWG_SLEEP_IN_WAIT_MS);
				}
				val &= ~0x03;
				shtps_rmi_write(ts, ts->map.fn01.ctrlBase, val);
				SHTPS_LOG_DBG_PRINT("[LPWG] device set doze [0x%02X]", val);
				if(SHTPS_LPWG_DOZE_IN_WAIT_MS > 0){
					mdelay(SHTPS_LPWG_DOZE_IN_WAIT_MS);
				}
			}else{
...
static void shtps_set_doze_wakeup_threshold(struct shtps_rmi_spi *ts)
{
	#if defined(SHTPS_DOZE_WAKEUP_THRESHOLD_CTRL_ENABLE)
	{
		u8 is_decision = 0;
		...
		#if defined( SHTPS_LOW_POWER_MODE_ENABLE )
			if(is_decision == 0){
				if((ts->lpmode_state & SHTPS_LPMODE_CONTINUOUS_REQ_ON_FOR_DTV) != 0){
					set_val = SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_DTV;
					is_decision = 1;
				}
				else if((ts->lpmode_state & SHTPS_LPMODE_CONTINUOUS_REQ_ON) != 0){
					set_val = SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_ECO;
					is_decision = 1;
				}
				else if((ts->lpmode_state & SHTPS_LPMODE_NON_CONTINUOUS_REQ_ON) != 0){
					set_val = SHTPS_DOZE_WAKEUP_THRESHOLD_LPMODE_DEF;
					is_decision = 1;
				}
			}
		#endif /* #if defined( SHTPS_LOW_POWER_MODE_ENABLE ) */
		...
		#if defined(SHTPS_LPWG_MODE_ENABLE)
			if(is_decision == 0){
				set_val = SHTPS_LPWG_DOZE_WAKEUP_THRESHOLD_DEF;
				is_decision = 1;
			}
		#endif /* SHTPS_LPWG_MODE_ENABLE */

		if(is_decision != 0){
			shtps_rmi_write(ts, ts->map.fn01.ctrlBase + 0x03, set_val);
			SHTPS_LOG_DBG_PRINT("set doze wakeup threshold : 0x%02X\n", set_val);
		}
	}
	#endif /* SHTPS_DOZE_WAKEUP_THRESHOLD_CTRL_ENABLE */
}
	...
	if(on){
		if((ts->lpmode_state & type) == 0){
			SHTPS_LOG_DBG_PRINT("%s(0x%02x) ON\n", 
				...
				(type == SHTPS_LPMODE_CONTINUOUS_REQ_ON_FOR_DTV)? "CONTINUOUS LPMODE for DTV" : "UNKNOWN ERROR", type);
			...
			shtps_set_doze_wakeup_threshold(ts);
		}
	}else{
		if((ts->lpmode_state & type) != 0){
...
static int shtps_set_hover_detect_enable(struct shtps_rmi_spi *ts)
{
	...
	if(ts->map.fn51.enable != 0){
		...
		data &= ~0x03;
		data |= 0x03;
		shtps_rmi_write(ts, ts->map.fn51.ctrlBase, data);
		shtps_rmi_write(ts, ts->map.fn51.ctrlBase + 5, SHTPS_HOVER_THRESH_FOR_ADC);
		shtps_rmi_read(ts, ts->map.fn51.ctrlBase + 6, &data, 1);
		data &= ~0x0F;
		data |= SHTPS_HOVER_TEMPORAL_FILTER;
		shtps_rmi_write(ts, ts->map.fn51.ctrlBase + 6, data);
...
static void shtps_set_lpwg_mode_on(struct shtps_rmi_spi *ts)
{
	...
	shtps_rmi_read_packet(ts, ts->map.fn12.ctrl.num[20].addr, data, 3);
	data[2] |= SHTPS_LPWG_REPORT_WG_ONLY;
	shtps_rmi_write_packet(ts, ts->map.fn12.ctrl.num[20].addr, data, 3);
	shtps_rmi_write(ts, 0xFF, ((ts->map.fn54.commandBase >> 8) & 0xFF));
	shtps_rmi_write(ts, ts->map.fn54.commandBase, 0x04);
	shtps_rmi_write(ts, 0xFF, 0x00);
...
void shtps_filter_touch_position_adjust_shift_edge(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
	...
	for(i = 0; i < fingerMax; i++){
		if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER && ts->report_info.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH){
			ts->shift_edge_inward.state[i] = SHTPS_SHIFT_EDGE_INWARD_STATE_IDLE;
			...
			if(info->fingers[i].x == 0){
				...
				ts->shift_edge_inward.state[i] |= SHTPS_SHIFT_EDGE_INWARD_STATE_ON_LEFT;
				...
				ts->shift_edge_inward.state[i] |= SHTPS_SHIFT_EDGE_INWARD_STATE_ON_RIGHT;
			}
			...
			if(info->fingers[i].y == 0){
				...
				ts->shift_edge_inward.state[i] |= SHTPS_SHIFT_EDGE_INWARD_STATE_ON_TOP;
				...
				ts->shift_edge_inward.state[i] |= SHTPS_SHIFT_EDGE_INWARD_STATE_ON_BOTTOM;
			}
		}else if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER){
			if((ts->shift_edge_inward.state[i] & SHTPS_SHIFT_EDGE_INWARD_STATE_ON_LEFT) != 0){
			...
			if((ts->shift_edge_inward.state[i] & SHTPS_SHIFT_EDGE_INWARD_STATE_ON_RIGHT) != 0){
			...
			if((ts->shift_edge_inward.state[i] & SHTPS_SHIFT_EDGE_INWARD_STATE_ON_TOP) != 0){
			...
			if((ts->shift_edge_inward.state[i] & SHTPS_SHIFT_EDGE_INWARD_STATE_ON_BOTTOM) != 0){
...
static void shtps_set_touch_info(struct shtps_rmi_spi *ts, u8 *buf, struct shtps_touch_info *info)
{
	...
	for(i = 0; i < fingerMax; i++){
		...
		if (SHTPS_RIPPLE_DISTORTION_REJECTION != 0) {
			...
			u8 size = sizeof(P);
			unsigned short before = info->fingers[i].y;

			info->fingers[i].y = (((before % (size * 2) < size) ? 1 : -1) * P[(before % size)]) + before;
			SHTPS_LOG_DBG_PRINT("[RIPPLE_DISTORTION_REJECTION] before Y[%d]=%d, after Y[%d]=%d\n", i, before, i, info->fingers[i].y);
		}
		#endif /* SHTPS_RIPPLE_DISTORTION_REJECTION_ENABLE */
	...
	#if defined(SHTPS_SHIFT_EDGE_INWARD_ENABLE)
		shtps_filter_touch_position_adjust_shift_edge(ts, info);
	#endif /* SHTPS_SHIFT_EDGE_INWARD_ENABLE */
}
...
static void shtps_touch_position_adjust(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
	#if defined(SHTPS_EDGE_POS_ADJUST_ENABLE)
	{
		int i;
		u8 fingerMax = shtps_get_fingermax(ts);
		unsigned short adjust_x;

		for(i = 0; i < fingerMax; i++){
			adjust_x = info->fingers[i].x;

			if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER){
				if( (0 <= info->fingers[i].x) && (info->fingers[i].x < SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A) )
				{
					adjust_x = ((SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A + SHTPS_EDGE_POS_ADJUST_VOLUME_S) * info->fingers[i].x) / SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A;
					SHTPS_LOG_DBG_PRINT("[pos_adjust][%d] area A detect\n", i);
				}
				else if( (SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A <= info->fingers[i].x) &&
						 (info->fingers[i].x < (SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_B)) )
				{
					adjust_x = info->fingers[i].x + SHTPS_EDGE_POS_ADJUST_VOLUME_S;
					SHTPS_LOG_DBG_PRINT("[pos_adjust][%d] area B detect\n", i);
				}
				else if( ((SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_B) <= info->fingers[i].x) &&
						 (info->fingers[i].x < (SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_B + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_C)) )
				{
					adjust_x = ( (((SHTPS_EDGE_POS_ADJUST_AREA_SIZE_C - SHTPS_EDGE_POS_ADJUST_VOLUME_S) * info->fingers[i].x) +
								 (SHTPS_EDGE_POS_ADJUST_VOLUME_S *
									(SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_B + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_C))) /
								 SHTPS_EDGE_POS_ADJUST_AREA_SIZE_C);
					SHTPS_LOG_DBG_PRINT("[pos_adjust][%d] area C detect\n", i);
				}
				else if( ((CONFIG_SHTPS_SY3000_LCD_SIZE_X -
						 (SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_B + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_C)) <= info->fingers[i].x) &&
						 (info->fingers[i].x < (CONFIG_SHTPS_SY3000_LCD_SIZE_X - (SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_B))) )
				{
					adjust_x = (((SHTPS_EDGE_POS_ADJUST_AREA_SIZE_C - SHTPS_EDGE_POS_ADJUST_VOLUME_S) * info->fingers[i].x) +
								(SHTPS_EDGE_POS_ADJUST_VOLUME_S *
									(CONFIG_SHTPS_SY3000_LCD_SIZE_X -
									(SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_B + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_C)))) /
								SHTPS_EDGE_POS_ADJUST_AREA_SIZE_C;
					SHTPS_LOG_DBG_PRINT("[pos_adjust][%d] area C~ detect\n", i);
				}
				else if( ((CONFIG_SHTPS_SY3000_LCD_SIZE_X -
						 (SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A + SHTPS_EDGE_POS_ADJUST_AREA_SIZE_B)) <= info->fingers[i].x) &&
						 (info->fingers[i].x < (CONFIG_SHTPS_SY3000_LCD_SIZE_X - SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A)) )
				{
					adjust_x = info->fingers[i].x - SHTPS_EDGE_POS_ADJUST_VOLUME_S;
					SHTPS_LOG_DBG_PRINT("[pos_adjust][%d] area B~ detect\n", i);
				}
				else if( ((CONFIG_SHTPS_SY3000_LCD_SIZE_X - SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A) <= info->fingers[i].x) &&
						 (info->fingers[i].x < CONFIG_SHTPS_SY3000_LCD_SIZE_X) )
				{
					adjust_x = (((SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A + SHTPS_EDGE_POS_ADJUST_VOLUME_S) * info->fingers[i].x) -
								(SHTPS_EDGE_POS_ADJUST_VOLUME_S * (CONFIG_SHTPS_SY3000_LCD_SIZE_X - 1))) / SHTPS_EDGE_POS_ADJUST_AREA_SIZE_A;
					SHTPS_LOG_DBG_PRINT("[pos_adjust][%d] area A~ detect\n", i);
				}

				if(info->fingers[i].x != adjust_x){
					SHTPS_LOG_DBG_PRINT("[pos_adjust][%d] x : %d -> %d\n", i, info->fingers[i].x, adjust_x);
					info->fingers[i].x = adjust_x;
				}
			}
		}
	}
...
static void shtps_key_event_report(struct shtps_rmi_spi *ts, u8 state)
{
	...
	if(ts->key_state != state){
		if( ((ts->key_state & (1 << SHTPS_PHYSICAL_KEY_MENU)) ^ (state & (1 << SHTPS_PHYSICAL_KEY_MENU))) != 0 ){
			input_report_key(ts->input_key, KEY_MENU, ((state >> SHTPS_PHYSICAL_KEY_MENU) & 0x01));
			isEvent = 1;
			SHTPS_LOG_EVENT(
				printk(KERN_DEBUG "[shtps][key]Notify event KEY_MENU:%s\n", (((state >> SHTPS_PHYSICAL_KEY_MENU) & 0x01) ? "DOWN" : "UP"));
			);
		}
		if( ((ts->key_state & (1 << SHTPS_PHYSICAL_KEY_HOME)) ^ (state & (1 << SHTPS_PHYSICAL_KEY_HOME))) != 0 ){
			input_report_key(ts->input_key, KEY_HOMEPAGE, ((state >> SHTPS_PHYSICAL_KEY_HOME) & 0x01));
			isEvent = 1;
			SHTPS_LOG_EVENT(
				printk(KERN_DEBUG "[shtps][key]Notify event KEY_HOMEPAGE:%s\n", (((state >> SHTPS_PHYSICAL_KEY_HOME) & 0x01) ? "DOWN" : "UP"));
			);
		}
		if( ((ts->key_state & (1 << SHTPS_PHYSICAL_KEY_BACK)) ^ (state & (1 << SHTPS_PHYSICAL_KEY_BACK))) != 0 ){
			input_report_key(ts->input_key, KEY_BACK, ((state >> SHTPS_PHYSICAL_KEY_BACK) & 0x01));
			isEvent = 1;
			SHTPS_LOG_EVENT(
				printk(KERN_DEBUG "[shtps][key]Notify event KEY_BACK:%s\n", (((state >> SHTPS_PHYSICAL_KEY_BACK) & 0x01) ? "DOWN" : "UP"));
			);
		}
	}
...
static void shtps_key_event_force_touchup(struct shtps_rmi_spi *ts)
{
	...
	if(ts->key_state != 0){
		if( (ts->key_state & (1 << SHTPS_PHYSICAL_KEY_MENU)) != 0 ){
			input_report_key(ts->input_key, KEY_MENU, 0);
			isEvent = 1;
			SHTPS_LOG_EVENT(
				printk(KERN_DEBUG "[shtps][key]Notify event KEY_MENU:UP\n");
			);
		}
		if( (ts->key_state & (1 << SHTPS_PHYSICAL_KEY_HOME)) != 0 ){
			input_report_key(ts->input_key, KEY_HOMEPAGE, 0);
			isEvent = 1;
			SHTPS_LOG_EVENT(
				printk(KERN_DEBUG "[shtps][key]Notify event KEY_HOMEPAGE:UP\n");
			);
		}
		if( (ts->key_state & (1 << SHTPS_PHYSICAL_KEY_BACK)) != 0 ){
			input_report_key(ts->input_key, KEY_BACK, 0);
			isEvent = 1;
			SHTPS_LOG_EVENT(
				printk(KERN_DEBUG "[shtps][key]Notify event KEY_BACK:UP\n");
			);
		}
	}
...
static void shtps_read_touchevent(struct shtps_rmi_spi *ts, int state)
{
	...
	#if defined(SHTPS_WAKEUP_FAIL_TOUCH_EVENT_REJECTION_ENABLE)
		if(ts->wakeup_fail_touch_check != 0){
			if((buf[1] & SHTPS_IRQ_ABS) == 0){
...
static int shtps_ioctl_set_continuous_low_power_mode_for_DTV(struct shtps_rmi_spi *ts, unsigned long arg)
{
#if defined( SHTPS_LOW_POWER_MODE_ENABLE )
	SHTPS_LOG_DBG_PRINT("mutex_lock()\n");
	mutex_lock(&shtps_ctrl_lock);

	shtps_set_lpmode(ts, SHTPS_LPMODE_CONTINUOUS_REQ_ON_FOR_DTV, (int)arg);
...
static long shtpsif_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TPSDEV_SET_CONT_LOWPOWER_MODE_FOR_DTV:	rc = shtps_ioctl_set_continuous_low_power_mode_for_DTV(ts, arg); break;
	default:
...
{
	#if defined(SHTPS_CPU_CLOCK_CONTROL_ENABLE) && defined(SHTPS_CREATE_KOBJ_ENABLE)
		sysfs_remove_group(&(shtpsif_cdev.kobj), &dev_attr_grp);
	#endif /* SHTPS_CPU_CLOCK_CONTROL_ENABLE && SHTPS_CREATE_KOBJ_ENABLE */
...
static int __devinit shtps_rmi_probe(struct spi_device *spi)
{
	...
	wake_lock_init(&ts->facetouch.wake_lock, WAKE_LOCK_SUSPEND, "shtps_wake_lock");
		...
		ts->perf_lock_level          = SHTPS_PERF_LOCK_LEVEL_HIGH;
		perf_lock_init(&ts->perf_lock_level_highest, PERF_LOCK_HIGHEST, "shtps_perf_lock_highest");
		perf_lock_init(&ts->perf_lock_level_high,    PERF_LOCK_1026MHz, "shtps_perf_lock_high");
	...
	#endif /* SHTPS_PEN_DETECT_ENABLE */
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, SHTPS_FINGER_WIDTH_PALMDET, SHTPS_FINGER_WIDTH_FUZZ, 0);
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y,  0, CONFIG_SHTPS_SY3000_LCD_SIZE_Y - 1, 0, 0);
		...
		for(i = 0; i < input_dev->mtsize; i++){
			input_mt_set_value(&input_dev->mt[i], ABS_MT_PRESSURE, 256);
		}
	#else
	...
	shtps_performance_check_init();
		...
		if(ts->kobj == NULL){
			...
			result = sysfs_create_group(ts->kobj, &attr_grp_ctrl);
			if(result){
