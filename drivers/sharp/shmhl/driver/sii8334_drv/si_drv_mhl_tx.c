...
bool_t SiiMhlTxChipInitialize (void)
{
	...
	if(first_flg) {
		...
		if(i2c_ok_flg) {
			...
			TX_DEBUG_PRINT(("Reg_Dump(DEFAULT) Start -------------------------------\n"));
			shmhl_SiI8334_Reg_Dump_All_Offset(NULL, 0);
			TX_DEBUG_PRINT(("Reg_Dump(DEFAULT) End -------------------------------\n"));
			WriteInitialRegisterValues();
			TX_DEBUG_PRINT(("Reg_Dump(AFTER INIT) Start -------------------------------\n"));
			shmhl_SiI8334_Reg_Dump_All_Offset(NULL, 0);
			TX_DEBUG_PRINT(("Reg_Dump(AFTER INIT) End -------------------------------\n"));
...
static void WriteInitialRegisterValues (void)
{
	...
	if (shmhl_reg_eye_pattern != 0x00) {
		...
		/* With SWIC */
		SiiRegWrite(REG_MHLTX_CTL4, 0xFC);
...
//      result = ERROR_CBUS_ABORT;		// No Retry will help
        ...
        if(shmhl_get_debug_flg(SHMHL_DEBUG_LOG_DUMP)) {
            TX_DEBUG_PRINT(("Reg_Dump(CBusProcessErrors) Start -------------------------------\n"));
            shmhl_SiI8334_Reg_Dump_All_Offset(NULL, 0);
            TX_DEBUG_PRINT(("Reg_Dump(CBusProcessErrors) End -------------------------------\n"));
        }
    }
    return( result );
}
