 ...
 *	Internal Define
 */
#define SHMHL_DUMP_LOG(buf, size, print_size, args...)	if(NULL == buf){printk("[SHMHL] " args);}else{print_size = snprintf(buf, size, args);}
...
static struct device*		shmhl_device = NULL;
...
static bool shmhl_mhl_chip_SiI8334 = true;
// ____________________________________________________________________________________________________________________________
/**
 *	Internal prototypes
 */
...
static void shmhl_debugboard_setting(void);
...
static int shmhl_reg_enable(const char *id, int min_uVol, int max_uVol)
{
	...
	struct device *dev = shmhl_device;
...
bool shmhl_is_SiI8334(void)
{
	SHMHL_DEBUG_LOG_L("%s: %d\n", __func__, shmhl_mhl_chip_SiI8334);
	return shmhl_mhl_chip_SiI8334;
}

void shmhl_check_mhl_chip_version(void)
{
	uint8_t hwRevision = sh_boot_get_hw_revision();
	SHMHL_DEBUG_LOG_L("%s: hwRevision:0x%02x\n", __func__, hwRevision);
	...
	switch(hwRevision) {
		case 0x00: /* REV_ES0 */
		case 0x01: /* REV_ES0.5 */
			shmhl_mhl_chip_SiI8334 = false;
			break;
		default:
			shmhl_mhl_chip_SiI8334 = true;
			break;
	}

	SHMHL_DEBUG_LOG_L("%s: shmhl_mhl_chip_SiI8334:%d\n", __func__, shmhl_mhl_chip_SiI8334);
}
...
static void shmhl_debugboard_setting(void)
{
	sharp_smem_common_type *smem_addr = NULL;
	
	SHMHL_DEBUG_LOG_L("%s: Start\n", __func__);
	
	smem_addr = sh_smem_get_common_address();
	if ( NULL == smem_addr ) {
		SHMHL_DEBUG_LOG_H("%s: sh_smem_get_common_address failed\n", __func__);
		return;
	}
	if (!smem_addr->sh_hw_handset) {
		SHMHL_DEBUG_LOG_L("%s: debug board setting\n", __func__);
#ifdef CONFIG_SII8334_MHL_TX_DEBUGBOARD_SETTING_TYPE1
		shmhl_reg_eye_pattern = 0xCF;
#endif /* CONFIG_SII8334_MHL_TX_DEBUGBOARD_SETTING_TYPE1 */
	}
...
static int __init shmhl_init(void)
{
	...
	shmhl_kdrv_dev = dev;
	shmhl_device = device_create(shmhl_kdrv_class, NULL, shmhl_kdrv_dev, NULL, SH_MHL_DEVICE_NAME);
	...
	shmhl_check_mhl_chip_version();

	shmhl_debugboard_setting();
