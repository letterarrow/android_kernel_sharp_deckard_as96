...
static int smd_ioctl_set_qrdata(unsigned long arg)
{
	int ret = 0;
	struct shdiag_qrdata qr_data_work;
	sharp_smem_common_type *sharp_smem;
	
	if(copy_from_user(&qr_data_work, (unsigned short __user *)arg, sizeof(struct shdiag_qrdata)) != 0)
	{
		printk("[SH]smd_ioctl_set_qrdata: cpy_to_user FAILE\n");
		ret = -EFAULT;
	}
	else
	{
		sharp_smem = sh_smem_get_common_address();
		memcpy( (void*)(sharp_smem->shdarea_QRData), qr_data_work.buf, SHDIAG_QRDATA_SIZE );
	}
...
static int smd_ioctl_set_qxdmflg(unsigned long arg)
{
	...
	{
		printk("[SH]smd_ioctl_set_qxdmflg: cpy_to_user FAILE\n");
		ret = -EFAULT;
	}
	else
	{
...
static int smd_ioctl_set_proadj(unsigned long arg)
{
	...
	{
		printk("[SH]smd_ioctl_set_proadj: cpy_to_user FAILE\n");
		ret = -EFAULT;
	}
	else
	{
...
static int smd_ioctl_set_hapticscal(unsigned long arg)
{
	...
	{
		printk("[SH]smd_ioctl_set_hapticscal: cpy_to_user FAILE\n");
		ret = -EFAULT;
	}
	else
	{
...
static int smd_ioctl_set_cprxhilow(unsigned long arg)
{
	int ret = 0;
	unsigned int stage = 0;
	struct shdiag_cprxhilow cprxhilow_work;
	sharp_smem_common_type *sharp_smem;
	
	if(copy_from_user(&cprxhilow_work, (unsigned short __user *)arg, sizeof(struct shdiag_cprxhilow)) != 0)
	{
		printk("[SH]smd_ioctl_set_cprxhilow: cpy_to_user FAILE\n");
		ret = -EFAULT;
	}
	else
	{
		sharp_smem = sh_smem_get_common_address();
		stage = cprxhilow_work.stage;
		
		memcpy( (void*)(sharp_smem->shdiag_cprx_offset_hl[stage]), cprxhilow_work.buf, SHDIAG_CPRXHILOW_SIZE_ONE);
	
	}
	
	return ret;
}
static int smd_ioctl_set_cprxcal(unsigned long arg)
{
	int ret = 0;
	unsigned int stage = 0;
	struct shdiag_cprxcal cprxcal_work;
	sharp_smem_common_type *sharp_smem;
	
	if(copy_from_user(&cprxcal_work, (unsigned short __user *)arg, sizeof(struct shdiag_cprxcal)) != 0)
	{
		printk("[SH]smd_ioctl_set_cprxcal: cpy_to_user FAILE\n");
		ret = -EFAULT;
	}
	else
	{
		sharp_smem = sh_smem_get_common_address();
		stage = cprxcal_work.stage;
		
		sharp_smem->shdiag_cprx_cal_offset[stage] = cprxcal_work.buf;
	}
...
static long smd_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
	case SHDIAG_IOCTL_SET_QRDATA:
		ret = smd_ioctl_set_qrdata(arg);
		break;
	...
	case SHDIAG_IOCTL_SET_HAPTICSCAL:
		...
		break;
	case SHDIAG_IOCTL_SET_CPRXHILOOFFSET:
		ret = smd_ioctl_set_cprxhilow(arg);
		break;
	case SHDIAG_IOCTL_SET_CPRXCALOFFSET:
		ret = smd_ioctl_set_cprxcal(arg);
		break;
	default:
