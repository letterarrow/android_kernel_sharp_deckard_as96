 ...
 *
 * $Id: wl_android.c 386965 2013-02-22 14:48:18Z $
 */
	...
	struct net_device *dev, char* command, int total_len)
{
...
#if 0 /* [WLAN][SHARP] 2012.12.11 fix up the findings of CppCheck, Start */
	...
	memset(country_code, 0, sizeof(country_code));
	sscanf(command+sizeof("SETCOUNTRYREV"), "%10s %10d", temp, &rev);
	memcpy(country_code, temp, WLC_CNTRY_BUF_SZ-1);
#endif /* [WLAN][SHARP] 2012.12.11 fix up the findings of CppCheck, End */
		...
		if (assoc_maclist->count) { 
			...
			for (i = 0; i < assoc_maclist->count; i++) {
				...
				for (j = 0; j < maclist->count; j++) {
					...
					if (memcmp(assoc_maclist->ea[i].octet, maclist->ea[j].octet, ETHER_ADDR_LEN) == 0) {
						match = 1;
						break;
					}
				} 
				...
				if ((macmode == MACLIST_MODE_ALLOW && !match) ||
						...
						if ((ret = wldev_ioctl(dev, WLC_SCB_DEAUTHENTICATE_FOR_REASON, 
...
wl_android_set_mac_address_filter(struct net_device *dev, const char* str)
{
	...
	str += ((macnum / 10 ? 1 : 0) + 1) + 1;
	/* allocate memory for the MAC list */
	list = (struct maclist*)kmalloc(sizeof(int) + sizeof(struct ether_addr) * macnum, GFP_KERNEL);
	if (!list) {
	...
	for (i = 0; i < list->count; i++) {
		if (!(ret = bcm_ether_atoe(str + 18 * i, &list->ea[i]))){
			DHD_ERROR(("%s : Parsing error=%d\n", __FUNCTION__, ret));
		}		
		DHD_INFO(("%s : macnum=%d macmode=%d addr="MACDBG "\n", 
		  __FUNCTION__, macnum, macmode, MAC2STRDBG(list->ea[i].octet)));
	} 
	/* set the list */
...
/* [WLAN][SHARP] 2012.02.17 Suppress duplicated execution of sdio_reset_comm() Start */
#if 0
			ret = sdioh_start(NULL, 0);
#endif
/* [WLAN][SHARP] 2012.02.17 Suppress duplicated execution of sdio_reset_comm() End */
...
#ifdef WL_CFG80211
/* [WLAN][SHARP] 2013.04.25 replace country code handling Start */
#if 0
...
#else 
	/* CONFIG_SHWLAN_COUNTRY_CODE_HANDLING feature is define for only GGSM model */
	else if (strnicmp(command, CMD_COUNTRY, strlen(CMD_COUNTRY)) == 0) {
		#ifdef CONFIG_SHWLAN_COUNTRY_CODE_HANDLING
		char *country_code = command + strlen(CMD_COUNTRY) + 1;
		bytes_written = wldev_set_country(net, country_code);
		WL_ERR(("Country code is defined '%s' \n", country_code));
		#else  /* CONFIG_SHWLAN_COUNTRY_CODE_HANDLING */
		WL_ERR(("Country code is not defined \n"));
		#endif /* CONFIG_SHWLAN_COUNTRY_CODE_HANDLING */
	}
#endif
/* [WLAN][SHARP] 2013.04.25 replace country code handling End */
#endif /* WL_CFG80211 */
...
/* [WLAN][SHARP] 2012.09.04 Tethering to limit the number of connections. Start */
	else if (strnicmp(command, CMD_SET_MAXASSOC, strlen(CMD_SET_MAXASSOC)) == 0) {
		uint maxassoc_val;
		sscanf(command + strlen(CMD_SET_MAXASSOC) + 1, "%d", &maxassoc_val);
		bytes_written = wldev_set_maxassoc(net, maxassoc_val);
	}
/* [WLAN][SHARP] 2012.09.04 Tethering to limit the number of connections. End */
	else {
...
/* [WLAN][SHARP] 2012.03.05 Customize iface_name "wlan" -> "eth" Start */
#if 0
		bcm_strncpy_s(iface_name, IFNAMSIZ, "wlan", IFNAMSIZ);
#else
		bcm_strncpy_s(iface_name, IFNAMSIZ, "eth", IFNAMSIZ);
#endif
/* [WLAN][SHARP] 2012.03.05 Customize iface_name "wlan" -> "eth" End */
	}
#endif /* CUSTOMER_HW2 || CUSTOMER_HW4 */
