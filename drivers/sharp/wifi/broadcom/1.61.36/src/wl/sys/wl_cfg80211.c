 ...
 *
 * $Id: wl_cfg80211.c 387893 2013-02-27 14:21:37Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
/* [WLAN][SHARP] 2013.04.04 Mod for Debug Log Start */
#if 0
#ifdef CUSTOMER_HW4
...
u32 wl_dbg_level = WL_DBG_ERR;
#endif
/* [WLAN][SHARP] 2013.04.04 Mod for Debug Log End */
...
/* [WLAN][SHARP] 2012.10.11 check invalid length Start */
#define SH_INVALID_BSS_LENGTH_THRESHOLD  10000
/* [WLAN][SHARP] 2012.10.11 check invalid length End */
...
/* [WLAN][SHARP] Customize Start
                  2012.09.12 add from 1.28.4
                  2012.10.10 RSSI Offset setting */
#ifndef RSSI_OFFSET
#ifdef  CONFIG_SHWLAN_RSSI_OFFSET
#define RSSI_OFFSET	CONFIG_SHWLAN_RSSI_OFFSET
#else
#define RSSI_OFFSET	0
#endif /* CONFIG_SHWLAN_RSSI_OFFSET */
#endif /* RSSI_OFFSET */
...
/* [WLAN][SHARP] 2012.02.13 Adjust debug log level Start */
//#if (WL_DBG_LEVEL > 0)
/* [WLAN][SHARP] 2012.02.13 Adjust debug log level End */
	...
	if (ieee80211_is_mgmt(mgmt->frame_control)) {
		if (ieee80211_is_probe_resp(mgmt->frame_control)) {
			...
			WL_ERR(("Disconnect STA : %s scb_val.val %d\n",
				...
				scb_val.val));
			if (num_associated) {
/* [WLAN][SHARP] 2012.12.20 Improvement of the issues that are disconnected from the GO in 4Way HS Start */
#if 0
				wl_delay(400);
#endif
/* [WLAN][SHARP] 2012.12.20 Improvement of the issues that are disconnected from the GO in 4Way HS End */
			}
			cfg80211_mgmt_tx_status(ndev, *cookie, buf, len, true, GFP_KERNEL);
			goto exit;
...
/* [WLAN][SHARP] 2013.01.22 Disable Country IE Start */
#ifdef DISABLE_11H_FOR_SOFTAP
	s32 spect = 0;
	channel_info_t ci;
	s32 channel;
#endif /* DISABLE_11H_FOR_SOFTAP */
/* [WLAN][SHARP] 2013.01.22 Disable Country IE End */
...
/* [WLAN][SHARP] 2013.01.22 Disable Country IE Start */
#ifdef DISABLE_11H_FOR_SOFTAP
		/* Disable on 2.4GHz band for compatibility with some STA. */
		if ((err = wldev_ioctl(dev, WLC_GET_CHANNEL, &ci, sizeof(ci), false))) {
		...
		if (channel <= CH_MAX_2G_CHANNEL) {
			...
			if (err < 0) {
				WL_ERR(("Setting spectrum management failed %d\n", err));
				goto exit;
			}
		}
#endif /* DISABLE_11H_FOR_SOFTAP */
	...
	s32 bssidx)
{
	...
	if (dhd->op_mode & DHD_FLAG_HOSTAP_MODE) {
		...
		struct ieee80211_mgmt *mgmt;
		mgmt = (struct ieee80211_mgmt *)info->probe_resp;
#if 0 /* [WLAN][SHARP] 2013.04.08 fix tethering problem (WPA, WPA2) Start */
		vndr = (u8 *)&mgmt->u.probe_resp.variable;
		vndr_ie_len = info->probe_resp_len -
			offsetof(struct ieee80211_mgmt, u.probe_resp.variable);
#else
...
#endif /* [WLAN][SHARP] 2013.04.08 fix tethering problem (WPA, WPA2) End */
	} else {
		/* Other mode */
	vndr = (u8 *)info->proberesp_ies;
	vndr_ie_len = info->proberesp_ies_len;
	...
	struct net_device *dev)
{
	int err = 0;
	u32 dev_role = 0;
/* [WLAN][SHARP] 2012.12.05 Mod for setting AP mode Start */
#if 0
	int infra = 0;
#endif
/* [WLAN][SHARP] 2012.12.05 Mod for setting AP mode End */
	int ap = 0;
	s32 bssidx = 0;
	struct wl_priv *wl = wiphy_priv(wiphy);
...
/* [WLAN][SHARP] 2012.10.25 initialization n_channels. Start */
	wdev->wiphy->bands[IEEE80211_BAND_5GHZ] = &__wl_band_5ghz_a;
	wdev->wiphy->bands[IEEE80211_BAND_2GHZ]->n_channels = ARRAY_SIZE(__wl_2ghz_channels);
	wdev->wiphy->bands[IEEE80211_BAND_5GHZ]->n_channels = ARRAY_SIZE(__wl_5ghz_a_channels);
/* [WLAN][SHARP] 2012.10.25 initialization n_channels. End */
		...
		if (wl_get_drv_status_all(wl, FINDING_COMMON_CHANNEL)) {
			...
			for (i = 0; i < list->count; i++) {
				bss = bss ? (wl_bss_info_t *)((uintptr)bss + dtoh32(bss->length))
					: list->bss_info;
/* [WLAN][SHARP] 2012.10.11 check invalid length Start */
				if( (dtoh32(bss->length))%2 ||
					(dtoh32(bss->length)) > SH_INVALID_BSS_LENGTH_THRESHOLD ){
					WL_ERR(("%s: dtoh32(bss->length)=[%d]\n", __func__, dtoh32(bss->length)));
					goto exit;
				}
/* [WLAN][SHARP] 2012.10.11 check invalid length End */
	...
	enum wl_status state, bool set)
{
	...
	if (set) {
		...
		if (wl->vsdb_mode) {
			err = wldev_iovar_setint(primary_dev, "tdls_auto_op", 0);
		}
...
#if defined(CUSTOMER_HW4) && defined(WLTDLS)
		if (!wl->vsdb_mode) {
			err = wldev_iovar_setint(primary_dev, "tdls_auto_op", 1);
		}
#endif /* defined(CUSTOMER_HW4) && defined(WLTDLS) */
	}
	return err;
...
static s32 __wl_cfg80211_down(struct wl_priv *wl)
{
	...
	struct net_info *iter, *next;
	struct net_device *ndev = wl_to_prmry_ndev(wl);
#if 0	/* [WLAN][SHARP] 2012.10.24 Disable P2P interface down Start */
#if defined(WL_CFG80211) && defined(SUPPORT_DEEP_SLEEP)
	struct net_device *p2p_net = wl->p2p_net;
#endif
#endif	/* [WLAN][SHARP] 2012.10.24 Disable P2P interface down End */
	u32 bssidx = 0;
	WL_DBG(("In\n"));
...
{
	if (set)
/* [WLAN][SHARP] 2012.03.27 Adjust debug log level Start */
#if 0
		wl_dbg_level = level & WL_DBG_LEVEL;
#else
		wl_dbg_level = level;
#endif
/* [WLAN][SHARP] 2012.03.27 Adjust debug log level End */
	else
