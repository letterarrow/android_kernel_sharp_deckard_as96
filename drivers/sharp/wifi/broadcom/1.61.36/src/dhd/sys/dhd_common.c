...
/* [WLAN][SHARP] 2012.03.27 Adjust debug log level Start */
		...
		if (!(int_val & DHD_WL_VAL2))
#else
		wl_cfg80211_enable_trace(TRUE, int_val);
#endif
...
#else /* [WLAN][SHARP] 2012.10.12 wifi eco mode */

#define SH_BCN_INTERVAL 600

#define DTIM_SKIP_NUM(bi, dtim) ((SH_BCN_INTERVAL / (bi * dtim)) == 0 ? \
								 1 : SH_BCN_INTERVAL / (bi * dtim))

#define BCN_SKIP_NUM(bi) ((SH_BCN_INTERVAL / bi) == 0 ?	\
						  1 : SH_BCN_INTERVAL / bi)

/* Function to estimate,set possible DTIM_SKIP BEACON_SKIP value */
int
dhd_set_beacon_dtim_skip(dhd_pub_t *dhd, bool enable)
{
	int ret = 0;
	int dtim_assoc = 0;
	int bi_assoc = 0;
	...
	if (enable) {
		if ((dhd->suspend_bcn_li_dtim == 0) || (dhd->suspend_bcn_li_dtim == 1))
			bcn_li_dtim = 0;
		else
			bcn_li_dtim = dhd->suspend_bcn_li_dtim;
		...
		/* if assoc grab ap's dtim */
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_DTIMPRD,
									&dtim_assoc, sizeof(dtim_assoc), FALSE, 0)) < 0) {
			DHD_ERROR(("%s DTIMPRD failed code %d\n", __FUNCTION__, ret));
			return ret;
		}

		/* if assoc grab ap's beacon interval */
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_BCNPRD,
									&bi_assoc, sizeof(bi_assoc), FALSE, 0)) < 0) {
			DHD_ERROR(("%s BCNPRD failed code %d\n", __FUNCTION__, ret));
			return ret;
		}

		/* if not associated just exit */
		if (dtim_assoc == 0 || bi_assoc == 0) {
			return -1;
		}

		if (bi_assoc * dtim_assoc > SH_BCN_INTERVAL) {
			bcn_li_bcn = BCN_SKIP_NUM(bi_assoc);
			bcn_li_dtim = 0;
			DHD_TRACE(("%s beacon_skip : receive interval =%d \n", 
						__FUNCTION__, bcn_li_bcn * bi_assoc));
		}

		if (bcn_li_dtim != 0 &&
			bi_assoc * dtim_assoc * bcn_li_dtim > SH_BCN_INTERVAL) {
			bcn_li_dtim = DTIM_SKIP_NUM(bi_assoc, dtim_assoc);
			bcn_li_bcn = 0;
			DHD_TRACE(("%s dtim_skip : receive interval =%d \n", 
						__FUNCTION__, bcn_li_dtim * bi_assoc * dtim_assoc));
		}
	} else {
		bcn_li_bcn = 0;
		bcn_li_dtim = 0;
	...
	DHD_ERROR(("%s DTIM=%d BI=%d bcn_li_dtim=%d bcn_li_bcn=%d \n", 
			 __FUNCTION__, dtim_assoc, bi_assoc, bcn_li_dtim, bcn_li_bcn));
