 ...
 *
 * $Id: dhd_sdio.c 387066 2013-02-22 23:21:45Z $
 */
...
/* [WLAN][SHARP] Customize Start
                 2011.09.07 Update the NVRAM with the read value by WCALREAD
                 2012.07.30 Calibration data for 5GHz
                 2012.08.28 Changed to retrieve the memory from the heap
                 2012.11.14 add nvram parameter btc_params6
                 2013.01.09 mod for BCM4335
                 2013.01.25 add item "dot11agofdmhrbw202gpo" */

#include "sharp/sh_smem.h"
#define SH_SMEM_READ_LEN_2G				30
#define SH_SMEM_READ_LEN_5G				100
#define SH_WLAN_CAL_ITEM_TLEN_2G		4
#define SH_WLAN_CAL_ITEM_TLEN_5G		10
#define SH_WLAN_CAL_ITEM_VALUE_SIZE		13
#define SH_LOG_LEN						80
#define SH_WLAN_CAL_ITEM_CONJUNCTION	"=0x"
#define SH_WLAN_CAL_ITEM_DEC_CONJUNCTION "="

#define SH_WLAN_CAL_ITEM_TO_BE_SEPARATED	"maxp5ga0"
#define SH_WLAN_CAL_SEPARATE_STR		","

enum {
#if defined(CONFIG_SHWLAN_NVRAM_BTC_PARAM6) && (CONFIG_SHWLAN_NVRAM_BTC_PARAM6 != 0)
	SH_WLAN_CAL_ITEM_TLEN_COMMON_BTC_PARAM6,
#endif	// defined(CONFIG_SHWLAN_NVRAM_BTC_PARAM6) && (CONFIG_SHWLAN_NVRAM_BTC_PARAM6 != 0)
	SH_WLAN_CAL_ITEM_TLEN_COMMON_MAX
};
...
sh_wlan_cal_item_t wlan_cal_item[SH_WLAN_CAL_ITEM_TLEN_2G
							   + SH_WLAN_CAL_ITEM_TLEN_5G
							   + SH_WLAN_CAL_ITEM_TLEN_COMMON_MAX
							   + 1] = {
	/* calibration data for 2.4GHz */
	{"maxp2ga0"					,	2,	0,	"" }, 
	{"ofdmlrbw202gpo"			,	4,	0,	"" }, 
	{"mcsbw202gpo"				,	8,	0,	"" }, 
	{"dot11agofdmhrbw202gpo"	,	4,	0,	"" }, 
	/* calibration data for 5GHz */
	{"maxp5ga0"					,	8,	0,	"" }, 
	{"mcsbw205glpo"				,	8,	0,	"" }, 
	{"mcsbw405glpo"				,	8,	0,	"" }, 
	{"mcsbw805glpo"				,	8,	0,	"" }, 
	...
	uint16 func, uint bustype, void *regsva, osl_t * osh, void *sdh)
{
	...
	dhd_doflow = FALSE;
#else
/* [WLAN][SHARP] 2013.03.27. Customized for wifi test Start */
#if 0
	dhd_doflow = TRUE;
#else
	dhd_doflow = FALSE;
#endif
/* [WLAN][SHARP] 2013.03.27. Customized for wifi test End */
#endif /* REPEAT_READFRAME */
...
/* [WLAN][SHARP] Customize start
				 2011.04.01 Mod for Static Link
				 2013.05.16 Suppress dhd_open execution in Wifi off sequence */
#ifdef SH_CONFIG_WLAN_STATIC 
#include <linux/rtnetlink.h>
extern void dhd_stop_thr_sysioc(dhd_pub_t *dhdp);
#endif
extern int dhd_loaded;
...
/* [WLAN][SHARP] 2011.04.01 Add for Static Link Start */
#ifdef SH_CONFIG_WLAN_STATIC 
		reject_ioctl = 1;
...
#endif
/* [WLAN][SHARP] 2011.04.01 Add for Static Link End */

		dhdsdio_release(bus, bus->dhd->osh);
	}

	/* [WLAN][SHARP] 2013.05.16 Suppress dhd_open execution in Wifi off sequence Start */
	dhd_loaded = FALSE;
	/* [WLAN][SHARP] 2013.05.16 Suppress dhd_open execution in Wifi off sequence End */
...
/* [WLAN][SHARP] Customize Start
                 ...
                 2012.07.30 Calibration data for 5GHz
                 2012.08.28 Changed to retrieve the memory from the heap
                 2012.10.19 Corresponding to the independent operation 2.4GHz/5GHz/
                 2012.11.14 add nvram parameter btc_params6 */
...
sh_nvram_debug_log(char *Head_line, char *Value_start, char *log_buf)
{
	...
	char *Value_startP = Value_start;

	if (*(Value_startP - 2) == '0' && *(Value_startP - 1) == 'x') {
		Value_startP -= 2;				/* two-byte back for '0x' */
	}
...
static bool
sh_cp_wlan_cal_item(sh_wlan_cal_item_t *wlan_cal_item, char *shdarea_WlanCalPAData, int sh_smem_read_len, int shdarea_WlanCalPAData_Str_TLen, struct dhd_bus *bus)
{
	int shdarea_WlanCalPAData_pos;
	bool check_shdarea_WlanCalPAData;
	int wlan_cal_item_len;
	char *shdarea_WlanCalPAData_StrP;
	...
	shdarea_WlanCalPAData_StrP = MALLOC(bus->dhd->osh, ((sh_smem_read_len * 2) + 1));

	if (shdarea_WlanCalPAData_StrP == NULL) {
		DHD_ERROR(("%s: Failed to allocate memory [%d * 2 + 1] bytes \n",
		           __FUNCTION__, sh_smem_read_len));
		return FALSE;
	} else {
		memset(shdarea_WlanCalPAData_StrP, '\0', ((sh_smem_read_len * 2) + 1));
	}

	while (shdarea_WlanCalPAData_pos < sh_smem_read_len) {
		sprintf(shdarea_WlanCalPAData_StrP + (shdarea_WlanCalPAData_pos * 2), "%02X", *(shdarea_WlanCalPAData + shdarea_WlanCalPAData_pos));
		shdarea_WlanCalPAData_pos++;
	}
	DHD_INFO(("%s: shdarea_WlanCalPAData_StrP all_data: %s\n",
				 __FUNCTION__, shdarea_WlanCalPAData_StrP));

	wlan_cal_itemP = wlan_cal_item;
	shdarea_WlanCalPAData_pos = 0;
	loop_cnt = 0;

	while (wlan_cal_itemP->name != NULL &&  loop_cnt < shdarea_WlanCalPAData_Str_TLen) {
		wlan_cal_item_total_size += wlan_cal_itemP->size;
		wlan_cal_item_len = 0;
		check_shdarea_WlanCalPAData = FALSE;
		while ((shdarea_WlanCalPAData_pos + wlan_cal_item_len) < wlan_cal_item_total_size) {
			if (shdarea_WlanCalPAData_StrP[shdarea_WlanCalPAData_pos + wlan_cal_item_len] != 'F') {
		...
		if (check_shdarea_WlanCalPAData == TRUE) {
			...
			check_shdarea_WlanCalPAData = FALSE;
			while ((shdarea_WlanCalPAData_pos + wlan_cal_item_len) < wlan_cal_item_total_size) {
				if (shdarea_WlanCalPAData_StrP[shdarea_WlanCalPAData_pos + wlan_cal_item_len] != '0') {
					check_shdarea_WlanCalPAData = TRUE;
					break;
				}
				wlan_cal_item_len++;
		...
		if (check_shdarea_WlanCalPAData ==TRUE &&
			wlan_cal_itemP->value[0] != 'X') {
			strncpy(wlan_cal_itemP->value, (shdarea_WlanCalPAData_StrP + shdarea_WlanCalPAData_pos), wlan_cal_itemP->size);
			wlan_cal_itemP->valid = 1;
			DHD_INFO(("%s: Name:%s Value:%s\n", __FUNCTION__,
				 wlan_cal_itemP->name, wlan_cal_itemP->value));
		}
	...
	MFREE(bus->dhd->osh, shdarea_WlanCalPAData_StrP, ((sh_smem_read_len * 2) + 1));
...
static bool
sh_modify_nvram_init(sh_wlan_cal_item_t *wlan_cal_item, struct dhd_bus *bus)
{
	sharp_smem_common_type *SMemCommAdrP;
	int loop_cnt;
	sh_wlan_cal_item_t *wlan_cal_itemP;
	bool ret = FALSE;
	bool common_data_exist = FALSE;
	bool smem_chk_flg;
...
#if defined(CONFIG_SHWLAN_NVRAM_BTC_PARAM6) && (CONFIG_SHWLAN_NVRAM_BTC_PARAM6 != 0)
	common_data_exist = TRUE;
	loop_cnt =  SH_WLAN_CAL_ITEM_TLEN_2G + SH_WLAN_CAL_ITEM_TLEN_5G
               +SH_WLAN_CAL_ITEM_TLEN_COMMON_BTC_PARAM6;
	wlan_cal_itemP[loop_cnt].valid = 1;
	sprintf(wlan_cal_itemP[loop_cnt].value, "%d", CONFIG_SHWLAN_NVRAM_BTC_PARAM6 );
#else
	DHD_INFO(("%s: btc_params6 does not set\n", __FUNCTION__));
#endif	// defined(CONFIG_SHWLAN_NVRAM_BTC_PARAM6) && (CONFIG_SHWLAN_NVRAM_BTC_PARAM6 != 0)


	/* get calibration data from SMEM*/
	...
	if (SMemCommAdrP == NULL) {
		...
		return (common_data_exist | ret);
	}

	/* for 2.4GHz */
	DHD_INFO(("%s : 2G data CHK!\n", __FUNCTION__));
	/* all 'F' check */
	loop_cnt = 0;
	smem_chk_flg = FALSE;
	while (loop_cnt < SH_SMEM_READ_LEN_2G) {
	...
	if (smem_chk_flg == FALSE) {
		...
		if (smem_chk_flg == FALSE) {
			/* 2.4GHz is NO DATA */
			wlan_cal_itemP = wlan_cal_item;
			while (wlan_cal_itemP < wlan_cal_item + SH_WLAN_CAL_ITEM_TLEN_2G) {
				wlan_cal_itemP->value[0] = 'X';
				wlan_cal_itemP->value[1] = '\0';
				wlan_cal_itemP++;
			}
		}
	}

	if (loop_cnt == SH_SMEM_READ_LEN_2G) {
		DHD_ERROR(("%s : Change value is not set!\n", __FUNCTION__));
		return (common_data_exist | ret);
	}
	
	wlan_cal_itemP = wlan_cal_item;
	ret = sh_cp_wlan_cal_item(wlan_cal_itemP, SMemCommAdrP->shdarea_WlanCalPAData, SH_SMEM_READ_LEN_2G, SH_WLAN_CAL_ITEM_TLEN_2G, bus);

	if (ret == FALSE) {
		DHD_ERROR(("%s : Calibration data did not copied!! (2.4GHz)\n", __FUNCTION__));
		return (common_data_exist | ret);
	}

	/* for 5GHz */
	DHD_INFO(("%s : 5G data CHK!\n", __FUNCTION__));
	/* all 'F' check */
	ret = FALSE;
	loop_cnt = 0;
	smem_chk_flg = FALSE;
	while (loop_cnt < SH_SMEM_READ_LEN_5G) {
		if (SMemCommAdrP->shdarea_WlanCal5GPAData[loop_cnt++] != 0xFF) {
			smem_chk_flg = TRUE;
			break;
		}
	}
	if (smem_chk_flg == FALSE) {
		...
		smem_chk_flg = FALSE;
		while (loop_cnt < SH_SMEM_READ_LEN_5G) {
			if (SMemCommAdrP->shdarea_WlanCal5GPAData[loop_cnt++] != 0x00) {
				smem_chk_flg = TRUE;
				break;
			}
		}
		if (smem_chk_flg == FALSE) {
			/* 5GHz is NO DATA */
			wlan_cal_itemP = wlan_cal_item + SH_WLAN_CAL_ITEM_TLEN_2G;
			while (wlan_cal_itemP < wlan_cal_item + SH_WLAN_CAL_ITEM_TLEN_2G + SH_WLAN_CAL_ITEM_TLEN_5G) {
				wlan_cal_itemP->value[0] = 'X';
				wlan_cal_itemP->value[1] = '\0';
				wlan_cal_itemP++;
			}
	...
	if (loop_cnt == SH_SMEM_READ_LEN_5G) {
		DHD_ERROR(("%s : Change value is not set!\n", __FUNCTION__));
		return (common_data_exist | ret);
	}
	wlan_cal_itemP = wlan_cal_item;
	ret = sh_cp_wlan_cal_item(wlan_cal_itemP + SH_WLAN_CAL_ITEM_TLEN_2G, SMemCommAdrP->shdarea_WlanCal5GPAData, SH_SMEM_READ_LEN_5G, SH_WLAN_CAL_ITEM_TLEN_5G, bus);

	if (ret == FALSE) {
		DHD_ERROR(("%s : Calibration data did not copied!! (5GHz)\n", __FUNCTION__));
	}
...
sh_modify_nvram_set_element(char **Head_lineP, char **End_lineP, char **memblock_valueP, sh_wlan_cal_item_t *wlan_cal_itemP)
{
	...
	*memblock_valueP = *Head_lineP + strlen(wlan_cal_itemP->name);

	if (strncmp(*memblock_valueP,SH_WLAN_CAL_ITEM_CONJUNCTION, strlen(SH_WLAN_CAL_ITEM_CONJUNCTION)) == 0) {
		*memblock_valueP += strlen(SH_WLAN_CAL_ITEM_CONJUNCTION);
	} else {
		*memblock_valueP += strlen(SH_WLAN_CAL_ITEM_DEC_CONJUNCTION);
	}
...
{
	uint ret_len;
	sh_nvram_item_t *sh_nvram_itemP;
	sh_wlan_cal_item_t *wlan_cal_itemP;
	sh_wlan_cal_item_t *org_wlan_cal_itemP;
	char buffer_separate[20];

	sh_nvram_itemP = MALLOC(bus->dhd->osh, sizeof(sh_nvram_item_t));
	if (sh_nvram_itemP) {
		memset(sh_nvram_itemP, 0, sizeof(sh_nvram_item_t));
	} else {
		return len;
	}
	...
	wlan_cal_itemP = wlan_cal_item;
	org_wlan_cal_itemP = wlan_cal_item;
	ret_len = len;

	sh_nvram_itemP->Head_lineP = memblock;
	sh_nvram_itemP->End_lineP = sh_nvram_itemP->Head_lineP;
	sh_nvram_itemP->memblock_valueP = NULL;
	sh_nvram_itemP->org_item_size=0;
	sh_nvram_itemP->mod_item_size=0;
	sh_nvram_itemP->add_str_len = 4; /* '=0x' & LF */
	sh_nvram_itemP->log_buf = NULL;

	if ((sh_modify_nvram_init(wlan_cal_itemP, bus)) == FALSE) {
		MFREE(bus->dhd->osh, sh_nvram_itemP, sizeof(sh_nvram_item_t));
		return ret_len;
	}

	if (dhd_msg_level & DHD_INFO_VAL){
		sh_nvram_itemP->log_buf = MALLOC(bus->dhd->osh, SH_LOG_LEN);
		if (sh_nvram_itemP->log_buf == NULL) {
			MFREE(bus->dhd->osh, sh_nvram_itemP, sizeof(sh_nvram_item_t));
			return ret_len;
		}
	}
    ...
    /* read line */
	while ((sh_nvram_itemP->Head_lineP - memblock) < ret_len) {

		wlan_cal_itemP = org_wlan_cal_itemP;

		while (wlan_cal_itemP->name != NULL) {
			if (wlan_cal_itemP->valid &&
			     (strncmp((sh_nvram_itemP->Head_lineP), wlan_cal_itemP->name, strlen(wlan_cal_itemP->name)) == 0)) {
				sh_nvram_itemP->org_item_size = sh_modify_nvram_set_element(&(sh_nvram_itemP->Head_lineP), &(sh_nvram_itemP->End_lineP),
												 &(sh_nvram_itemP->memblock_valueP),  wlan_cal_itemP);
				if (strncmp(wlan_cal_itemP->name, SH_WLAN_CAL_ITEM_TO_BE_SEPARATED, strlen(SH_WLAN_CAL_ITEM_TO_BE_SEPARATED)) == 0) {
					sh_nvram_itemP->mod_item_size = (wlan_cal_itemP->size + (strlen(SH_WLAN_CAL_SEPARATE_STR)*3))
					                                 - sh_nvram_itemP->org_item_size;
				} else {
					sh_nvram_itemP->mod_item_size = wlan_cal_itemP->size - sh_nvram_itemP->org_item_size;
				}
				/* Processing to outpu debug log */
				if (sh_nvram_itemP->log_buf != NULL) {
					memset(sh_nvram_itemP->log_buf, '\0', SH_LOG_LEN);
					sh_nvram_debug_log(sh_nvram_itemP->Head_lineP, sh_nvram_itemP->memblock_valueP, sh_nvram_itemP->log_buf);
				}

				/* Travels the memory, if necessary */
				if (sh_nvram_itemP->mod_item_size) {
					sh_nvram_memmove(&(sh_nvram_itemP->End_lineP), wlan_cal_itemP,
										 sh_nvram_itemP->mod_item_size, ret_len, memblock);
					ret_len += sh_nvram_itemP->mod_item_size;
					sh_nvram_itemP->End_lineP += sh_nvram_itemP->mod_item_size;
				}
				
				/* copied modify data */
				if ((strncmp(wlan_cal_itemP->name, SH_WLAN_CAL_ITEM_TO_BE_SEPARATED, strlen(SH_WLAN_CAL_ITEM_TO_BE_SEPARATED)) == 0) &&
				    (wlan_cal_itemP->size == 8)) {
					sprintf(buffer_separate, "%c%c%s%c%c%s%c%c%s%c%c"
								,wlan_cal_itemP->value[0] ,wlan_cal_itemP->value[1] ,SH_WLAN_CAL_SEPARATE_STR
								,wlan_cal_itemP->value[2] ,wlan_cal_itemP->value[3] ,SH_WLAN_CAL_SEPARATE_STR
								,wlan_cal_itemP->value[4] ,wlan_cal_itemP->value[5] ,SH_WLAN_CAL_SEPARATE_STR
								,wlan_cal_itemP->value[6] ,wlan_cal_itemP->value[7]);
					strncpy(sh_nvram_itemP->memblock_valueP, buffer_separate, strlen(buffer_separate));
				} else {
					strncpy(sh_nvram_itemP->memblock_valueP, wlan_cal_itemP->value, wlan_cal_itemP->size);
				}
				
				/* Processing to output debug log */
				if (sh_nvram_itemP->log_buf != NULL) {
					sh_nvram_debug_log(NULL, sh_nvram_itemP->memblock_valueP, (sh_nvram_itemP->log_buf + strlen(sh_nvram_itemP->log_buf)));
					DHD_INFO(("%s", sh_nvram_itemP->log_buf));
				}

				sh_nvram_itemP->Head_lineP = sh_nvram_itemP->End_lineP;
				wlan_cal_itemP->valid = 0;
		...
		while (*(sh_nvram_itemP->Head_lineP++) != 0x0A);
		
		if (*sh_nvram_itemP->Head_lineP == '\0') {
			break;
		}
	}

	/* newline code is CR/LF */
	if (*(sh_nvram_itemP->Head_lineP - 2) == 0x0d) {
		sh_nvram_itemP->add_str_len += 1;
	}

	wlan_cal_itemP = org_wlan_cal_itemP;

	while (wlan_cal_itemP->name != NULL) {
		if (wlan_cal_itemP->valid != 0) {

			char *log_startP = sh_nvram_itemP->Head_lineP;
			ret_len += sh_modify_nvram_add_element(memblock, sh_nvram_itemP->Head_lineP, wlan_cal_itemP);
			DHD_INFO(("%s: len+=[%d], Add to NVRAM [%s]\n", __FUNCTION__,
							 strlen(wlan_cal_itemP->name) + strlen(SH_WLAN_CAL_ITEM_CONJUNCTION)
							  + wlan_cal_itemP->size + sh_nvram_itemP->add_str_len,
							 log_startP));
		}
		wlan_cal_itemP++;
	}
	...
	if (sh_nvram_itemP->log_buf != NULL) {
		MFREE(bus->dhd->osh, sh_nvram_itemP->log_buf, SH_LOG_LEN);
	}
...
dhdsdio_download_nvram(struct dhd_bus *bus)
{
	...
	/* Download variables */
	if (nvram_file_exists) {
/* [WLAN][SHARP] 2011.09.07 Update the NVRAM with the read value by WCALREAD Start */
#if 0
...
#endif
/* [WLAN][SHARP] 2011.09.07 Update the NVRAM with the read value by WCALREAD End */
	}
	else {
...
/* [WLAN][SHARP] 2012.12.10 Mod for Null access ERROR fix Start */
	if ((bus == NULL) || (bus->dhd == NULL)){
		bcmerror = BCME_SDIO_ERROR;
		if (bus == NULL){
			DHD_ERROR(("%s: bus Interface is NULL %d\n", __FUNCTION__, flag));
		} else {
			DHD_ERROR(("%s: dhd Interface is NULL %d\n", __FUNCTION__, flag));
		}
		return bcmerror;
	}
/* [WLAN][SHARP] 2012.12.10 Mod for Null access ERROR fix End */
	if (flag == TRUE) {
		if (!bus->dhd->dongle_reset) {
			dhd_os_sdlock(dhdp);
