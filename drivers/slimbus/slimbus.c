...
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER) /*03-001*/
	dev_crit(&ctrl->dev, "Set LA %d for %02x:%02x:%02x:%02x:%02x:%02x\n",
		 i,
		 ctrl->addrt[i].eaddr[0],
		 ctrl->addrt[i].eaddr[1],
		 ctrl->addrt[i].eaddr[2],
		 ctrl->addrt[i].eaddr[3],
		 ctrl->addrt[i].eaddr[4],
		 ctrl->addrt[i].eaddr[5]);
#endif /* CONFIG_MFD_WM5102 && CONFIG_SH_AUDIO_DRIVER */ /*03-001*/
...
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER) /*03-001*/
int slim_change_val_elements(struct slim_device *sb, const void *r, size_t reg_size,
				    const void *val, size_t val_size)
{
	extern int msm_xfer_msg2(struct slim_controller *ctrl, u16 la, const void *r, size_t reg_size,
				    const void *val, size_t val_size);
	...
	if (sb->laddr != SLIM_LA_MANAGER && sb->laddr >= ctrl->num_dev)
		return -ENXIO;

	ret = msm_xfer_msg2(ctrl, sb->laddr, r, reg_size, val, val_size);

	return ret;
}
EXPORT_SYMBOL_GPL(slim_change_val_elements);
#endif /* CONFIG_MFD_WM5102 && CONFIG_SH_AUDIO_DRIVER */ /*03-001*/
...
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER) /*03-001*/
int slim_force_max_control_rate(struct slim_device *sb, u32 *prev_clkgear)
{
	struct slim_controller *ctrl = sb->ctrl;

	u32 clkgear = SLIM_MAX_CLK_GEAR;
	int ret = 0;
	u8 wbuf[4];

	/* stash the old values so we can restore */
	if (!prev_clkgear)
		ret = -1;

	*prev_clkgear = ctrl->clkgear;
	...
	if (ret == 0) {
		ret = slim_processtxn(ctrl, SLIM_MSG_DEST_BROADCAST,
			SLIM_MSG_MC_BEGIN_RECONFIGURATION, 0, SLIM_MSG_MT_CORE,
			NULL, NULL, 0, 3, NULL, 0, NULL);
		dev_err(&ctrl->dev, "sending begin_reconfig:ret:%d\n", ret);
	}
	if (!ret) {
		wbuf[0] = (u8)(clkgear & 0xFF);
		ret = slim_processtxn(ctrl, SLIM_MSG_DEST_BROADCAST,
			SLIM_MSG_MC_NEXT_CLOCK_GEAR, 0, SLIM_MSG_MT_CORE,
			NULL, wbuf, 1, 4, NULL, 0, NULL);
		dev_err(&ctrl->dev, "sending clkgear:%d,ret:%d\n",
				(int)wbuf[0], ret);
	}
	if (!ret) {
		ret = slim_processtxn(ctrl, SLIM_MSG_DEST_BROADCAST,
...
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER) /*03-001*/
int slim_restore_control_rate(struct slim_device *sb, u32 clkgear)
{
	struct slim_controller *ctrl = sb->ctrl;
	...
	if (clkgear < SLIM_MIN_CLK_GEAR || clkgear > SLIM_MAX_CLK_GEAR)
		ret = -1;
			...
			enum slim_ch_control chctrl, bool commit)
{
	...
	do {
		...
		slc = &ctrl->chans[chan];
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER) /*03-001*/
		dev_dbg(&ctrl->dev, "control_ch [chan:%d, ctrl:%d, def:%d, state:%d]\n", chan,
					chctrl, slc->def, slc->state);
		...
		if (chctrl == SLIM_CH_SUSPEND) {
			...
			if (slc->state < SLIM_CH_ACTIVE) {
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER) /*03-001*/
				dev_info(&sb->dev, "Removing inactive channel\n");
#endif /* CONFIG_MFD_WM5102 && CONFIG_SH_AUDIO_DRIVER */ /*03-001*/
				ret = -ENOTCONN;
				break;
			}
