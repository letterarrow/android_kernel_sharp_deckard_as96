...
#else  /* CONFIG_MFD_WM5102 && CONFIG_SH_AUDIO_DRIVER */ /*03-001*/
		if (dev->wr_comp)
			complete(dev->wr_comp);
#endif /* CONFIG_MFD_WM5102 && CONFIG_SH_AUDIO_DRIVER */ /*03-001*/
	}
...
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER) /*03-001*/
	if (mc == SLIM_MSG_MC_CONNECT_SOURCE) {
		dev_err(dev->dev, "Connect SRC [LA:%d] [port:%d] [chan:%d]\n",
			la, txn->wbuf[0], txn->wbuf[1]);
	}
	if (mc == SLIM_MSG_MC_CONNECT_SINK) {
		dev_err(dev->dev, "Connect SINK [LA:%d] [port:%d] [chan:%d]\n",
			la, txn->wbuf[0], txn->wbuf[1]);
	}
	if (mc == SLIM_MSG_MC_DISCONNECT_PORT) {
		dev_err(dev->dev, "Disonnect port [LA:%d] [port:%d]\n",
			la, txn->wbuf[0]);
	}
	if (mc == SLIM_MSG_MC_NEXT_CLOCK_GEAR) {
		dev_err(dev->dev, "Next Clock Gear: %d\n", txn->wbuf[0]);
	}
	if (mc == SLIM_MSG_MC_NEXT_SUBFRAME_MODE) {
		dev_err(dev->dev, "Next Subframe Mode: %d\n", txn->wbuf[0]);
	}
	if (mc == SLIM_MSG_MC_RECONFIGURE_NOW) {
		dev_err(dev->dev, "Reconfigure Now!\n");
	}
#endif /* CONFIG_MFD_WM5102 && CONFIG_SH_AUDIO_DRIVER */ /*03-001*/
...
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER) /*03-001*/
int msm_xfer_msg2(struct slim_controller *ctrl, u16 la, const void *r, size_t reg_size,
				    const void *val, size_t val_size)
{
#define NUM_BYTES_HEADER	5
#define NUM_BYTES_DATA		2
#define NUM_BYTES_PER_TRANS	(NUM_BYTES_HEADER + NUM_BYTES_DATA) // for padding
	DECLARE_COMPLETION_ONSTACK(done);
	struct msm_slim_ctrl *dev = slim_get_ctrldata(ctrl);
	u32 hd;
	u32 *pbuf;
	...
	msgv = msm_slim_get_ctrl(dev);
	mutex_lock(&dev->tx_lock);
	if (dev->state == MSM_CTRL_ASLEEP || dev->state == MSM_CTRL_SLEEPING) {
		dev_err(dev->dev, "runtime or system PM suspended state");
		mutex_unlock(&dev->tx_lock);
		if (msgv >= 0)
			msm_slim_put_ctrl(dev);
	...
	pbuf = msm_get_msg_buf(ctrl, NUM_BYTES_PER_TRANS);
	hd = SLIM_MSG_ASM_FIRST_WORD(NUM_BYTES_PER_TRANS, SLIM_MSG_MT_CORE,
				     SLIM_MSG_MC_CHANGE_VALUE, 0, la);
	reg = be32_to_cpup(r);

	while (val_size > 0) {
		sid[0] = 0x8009;
		sid[1] = 0xa009 + ((reg & 0xff) * 2 * 16);
		buf[0] = (reg >> 8) & 0xffff;
		buf[1] = be16_to_cpup(dat);
		...
		if (pag != buf[0]) {
			...
			*(puc++) = (buf[0] >> 8) & 0xFF;
			...
			dev->wr_comp = &done;
//			msm_send_msg_buf(ctrl, pbuf, NUM_BYTES_PER_TRANS);
			writel_relaxed(pbuf[0], dev->base + MGR_TX_MSG + 0);
			writel_relaxed(pbuf[1], dev->base + MGR_TX_MSG + 4);
			/* Guarantee that message is sent before returning */
			mb();
			...
			if (!timeout) {
				dev_err(dev->dev, "TX timed out:MC:0x%x,MT:0x%x",
						SLIM_MSG_MC_CHANGE_VALUE, SLIM_MSG_MT_CORE);
				dev->wr_comp = NULL;
			}
		}
