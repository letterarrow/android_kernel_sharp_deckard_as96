...
static const struct pm8xxx_adc_map_pt adcmap_pa_therm_type_x[] = {
	{1800,	-46},
	{1638,	-20},
	{1536,	-10},
	{1386,	  0},
...
#ifdef CONFIG_BATTERY_SH
static const struct pm8xxx_adc_map_pt adcmap_batt_therm_type_c[] = {
	{1800,	-40},
	{1615,	-20},
	{1514,	-10},
	{1389,	  0},
...
{
	*ptr_table = adcmap_batt_therm_type_c;
	*ptr_table_size = ARRAY_SIZE(adcmap_batt_therm_type_c);
}

static void pm8xxx_adc_scale_get_pa_therm_table( const struct pm8xxx_adc_map_pt **ptr_table, int *ptr_table_size )
{
	*ptr_table = adcmap_pa_therm_type_x;
	*ptr_table_size = ARRAY_SIZE(adcmap_pa_therm_type_x);
}
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
			offset = ((int64_t)XoAdc_volmin * (int64_t)XoAdc_admax) - ((int64_t)XoAdc_volmax * (int64_t)XoAdc_admin);
#else  /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
	int result = 0;
	result = pm8xxx_adc_scale_default(adc_code, adc_properties, chan_properties, adc_chan_result);
	if (result) {
		pr_err("Error %d pm8xxx_adc_scale_default %d,0x%x,0x%x,0x%x\n", result, adc_code, (unsigned int)adc_properties, (unsigned int)chan_properties, (unsigned int)adc_chan_result);
		return -EINVAL;
	}
#else  /* CONFIG_BATTERY_SH */
