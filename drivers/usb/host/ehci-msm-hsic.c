...
#ifdef CONFIG_USB_ANDROID_SH_CUST
static ssize_t irq_thresh_store(struct device *device, 
				...
				const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%04x", &value) == 1) {
		irq_thresh = value;
		ehci_irq_thresh_restore(dev_get_drvdata(device), irq_thresh);
...
static ssize_t
irq_thresh_show(struct device *sdev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", irq_thresh);
}

static DEVICE_ATTR(irq_thresh_sh, S_IRUGO | S_IWUSR, irq_thresh_show, irq_thresh_store);
#endif /* CONFIG_USB_ANDROID_SH_CUST */
...
static int __devinit ehci_hsic_msm_probe(struct platform_device *pdev)
{
	...
	if (pdata)
		...
		irq_thresh = pdata->log2_irq_thresh;
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
	ret = device_create_file(&pdev->dev, &dev_attr_irq_thresh_sh);
	if (ret < 0)
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
	/* Delete this debug snippet later */ 
	if (!pm_runtime_suspended(dev)) 
		dev_err(dev, "device is runtime active during PM suspend\n"); 
#endif /* CONFIG_USB_ANDROID_SH_CUST */
