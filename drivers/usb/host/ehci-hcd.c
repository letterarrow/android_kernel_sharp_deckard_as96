...
static void ehci_irq_thresh_restore(struct usb_hcd *hcd, unsigned irq_thresh)
{
	struct ehci_hcd		*ehci = hcd_to_ehci(hcd);

	pr_info("%s: irq_thresh=%d\n", __func__, irq_thresh);
	log2_irq_thresh = irq_thresh;

	if (log2_irq_thresh < 0 || log2_irq_thresh > 6)
		log2_irq_thresh = 0;

	ehci_readl(ehci, &ehci->regs->command);
	pr_info("%s: ehci->command=%x\n", __func__, ehci->command);

	ehci->command |= log2_irq_thresh << 16;
	ehci_writel(ehci, ehci->command, &ehci->regs->command);
