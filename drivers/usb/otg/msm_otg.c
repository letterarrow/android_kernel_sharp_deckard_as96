...
#ifdef CONFIG_USB_MSM_OTG_GPIO42
#include <linux/mfd/pm8xxx/pm8921.h>
#include <mach/gpio.h>
#include <mach/gpiomux.h>
#include <mach/irqs.h>
#endif /* CONFIG_USB_MSM_OTG_GPIO42 */
...
#ifndef CONFIG_USB_MSM_OTG_GPIO42
...
#ifndef CONFIG_USB_MSM_OTG_DISABLE_INTERNAL_ID_PULLUP
		/* Enable PMIC pull-up */
		pm8xxx_usb_id_pullup(1);
#endif /* CONFIG_USB_MSM_OTG_DISABLE_INTERNAL_ID_PULLUP */
	}
...
#ifdef CONFIG_USB_MSM_OTG_GPIO42
static int msm_otg_pm_gpio42_out(int outvalue)
{
	int ret = -EFAULT;
	int gpio_ret = 0;
	int pm_port_no = 0;
	...
	pr_debug("%s: Start [port_no=%d] [outvalue=%d]\n", __func__, 42, outvalue);

	gpio_param = &gpio42_param;

	if (NULL == gpio_param) {
		pr_debug("%s: Err Param\n", __func__);
		return ret;
	}

	pm_port_no = PM8921_GPIO_PM_TO_SYS(42);

	gpio_ret = gpio_request(pm_port_no, "vbus_otg");

	if (gpio_ret) {
		pr_debug("%s: Failed to request gpio_ret=%d\n", __func__, gpio_ret);
	...
	if (gpio_ret) {
		pr_debug("%s: Failed to config gpio_ret=%d\n", __func__, gpio_ret);
		gpio_free(pm_port_no);
		return ret;
	}
	...
	if (gpio_ret) {
		pr_debug("%s: Failed to output gpio_ret=%d\n", __func__, gpio_ret);
		gpio_free(pm_port_no);
		return ret;
	}
...
static void msm_hsusb_vbus_power(struct msm_otg *motg, bool on)
{
	...
	if (on) {
#ifdef CONFIG_USB_MSM_OTG_EXT5V_EXCLUSIVE
		ret = tps_driver_register_user(TPS_DRIVER_USER_USB);
		if (ret != TPS_DRIVER_SUCCESS) {
...
#else /* CONFIG_USB_MSM_OTG_GPIO42 */
		msm_otg_pm_gpio42_out(1);
#endif /* CONFIG_USB_MSM_OTG_GPIO42 */
		vbus_is_on = true;
	} else {
#ifndef CONFIG_USB_MSM_OTG_GPIO42
...
#else /* CONFIG_USB_MSM_OTG_GPIO42 */
		msm_otg_pm_gpio42_out(0);
#endif /* CONFIG_USB_MSM_OTG_GPIO42 */
#ifdef CONFIG_USB_MSM_OTG_EXT5V_EXCLUSIVE
		ret = tps_driver_unregister_user(TPS_DRIVER_USER_USB);
		if (ret != TPS_DRIVER_SUCCESS) {
...
#ifdef CONFIG_SHSYS_CUST_PERFLOCK
	limit_lock_init(&usb_host_limit_lock, PERF_LOCK_810MHz, DRIVER_NAME);
#endif /* CONFIG_SHSYS_CUST_PERFLOCK */
#endif /* CONFIG_USB_MSM_OTG_SH_CUST */
#ifdef CONFIG_USB_SH_CUST_NON_STANDARD_CHARGE
