...
#ifdef CONFIG_USE_LONG_TIMEOUT_MMC_CUST_SH
#define MMC_BLK_TIMEOUT_MS  (10 * 60 * 1000)        /* 10 minute timeout */
#endif /* CONFIG_USE_LONG_TIMEOUT_MMC_CUST_SH */
...
#endif /* MMC_CUST_SH && CONFIG_ANDROID_ENGINEERING */
...
static int sh_mmc_sd_status_checked;
static unsigned int sh_mmc_sd_status;
...
void mmc_detection_status_update(struct mmc_host *host, unsigned int status)
{
    if (strncmp(mmc_hostname(host), HOST_MMC_SD, sizeof(HOST_MMC_SD)) != 0)
        return;

    sh_mmc_sd_status_checked = 1;
    sh_mmc_sd_status = status;
...
int mmc_detection_status_check(struct mmc_host *host)
{
    ...
    if (sh_mmc_sd_status_checked)
        return sh_mmc_sd_status;
                          ...
                          struct mmc_command *cmd, sh_mmc_batlog_err_type type)
{
    ...
    if ((sh_mmc_sd_battlog_err_cmd != 0) || (sh_mmc_sd_battlog_err_type != 0)) {
        if (sh_mmc_battlog_debug_mask) {
            pr_info("%s: cmd & type are already set. (%d, %d, %d) (%d, %d)\n", __func__,
                    sh_mmc_sd_battlog_err_cmd, sh_mmc_sd_battlog_err_type,
                    sh_mmc_sd_battlog_err_ignore,
                    cmd->opcode, type);
		}
		return;
    }
    ...
    if (sh_mmc_battlog_debug_mask) {
        pr_info("%s: set cmd = %d -> %s (%d), type = %s (%d), ignore = %d\n", __func__,
                cmd->opcode, sh_mmc_batlog_err_cmd_str[err_cmd], err_cmd,
                sh_mmc_batlog_err_type_str[type], type, ignore);
    }
}
        ...
        if (!sh_mmc_sd_battlog_err_ignore) {
			if (sh_mmc_battlog_debug_mask) {
				pr_info("%s: %s: %s (%d), %s (%d), %s (%d), elapsed_ms = %lu\n",
                        __func__, (post ? "post" : "post canceled"),
						sh_mmc_batlog_err_cmd_str[sh_mmc_sd_battlog_err_cmd], sh_mmc_sd_battlog_err_cmd,
                        sh_mmc_batlog_err_type_str[sh_mmc_sd_battlog_err_type], sh_mmc_sd_battlog_err_type,
                        sh_mmc_batlog_err_result_str[result], result,
                        elapsed_ms);
			}
            if (post) {
	...
	if (!mmc_host_is_spi(card->host) && rq_data_dir(req) != READ) {
...
#else /* CONFIG_USE_LONG_TIMEOUT_MMC_CUST_SH */
#ifdef CONFIG_MMC_SD_CUST_SH
		int i = 0;
		int sleepy = 1;
		unsigned int msec = 0;
		...
		do {
            ...
            if( (status & R1_WP_VIOLATION) || 
                (sh_stop_resp_err & R1_WP_VIOLATION) ){
                pr_err( "%s: detect WP_VIOLATION(CMD%d,arg=0x%x,status=0x%x,stop_resp=0x%x)\n",
                        mmc_hostname(card->host), brq->cmd.opcode, 
                        brq->cmd.arg, status, sh_stop_resp_err );
                return MMC_BLK_NOMEDIUM;
            }
#endif /* CONFIG_RW_PROTECT_EMMC_CUST_SH */
			 ...
			 */
#ifndef CONFIG_USE_LONG_TIMEOUT_MMC_CUST_SH
#ifdef CONFIG_MMC_SD_CUST_SH
			i++;
#endif /* CONFIG_MMC_SD_CUST_SH */
#endif /* CONFIG_USE_LONG_TIMEOUT_MMC_CUST_SH */
...
#if defined(CONFIG_MMC_CUST_SH) && defined(CONFIG_ANDROID_ENGINEERING)
	if (sh_mmc_debug_mask && card && ((card)->type == (sh_mmc_debug_mask-1))) {
