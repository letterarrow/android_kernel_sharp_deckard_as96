...
static int sdpow_en_mpp = PM8921_MPP_PM_TO_SYS(4);
#endif /* CONFIG_MMC_SD_CUST_SH */
...
#if defined(CONFIG_USE_HS200_EMMC_CUST_SH) || defined(CONFIG_USE_UHS1_SD_CUST_SH)
static int msmsdcc_execute_tuning(struct mmc_host *mmc, u32 opcode);
#endif /* CONFIG_USE_HS200_EMMC_CUST_SH || CONFIG_USE_UHS1_SD_CUST_SH */
...
#ifdef CONFIG_PM_EMMC_CUST_SH
static inline void sh_msmsdcc_delay(unsigned int ms)
{
	if (ms < 1000 / HZ) {
		cond_resched();
		mdelay(ms);
...
#else /* CONFIG_USE_HS200_EMMC_CUST_SH || CONFIG_USE_UHS1_SD_CUST_SH */
	if (status & MCI_DATACRCFAIL) {
		if (!(data->mrq->cmd->opcode == MMC_BUS_TEST_W
			...
			data->error = -EILSEQ;
#ifdef  CONFIG_MMC_SD_BATTLOG_CUST_SH
            mmc_set_err_cmd_type(host->mmc, data->mrq->cmd, _CRC_ERROR);
#endif
		}
		...
		if (!(data->mrq->cmd->opcode == MMC_BUS_TEST_W
			...
			msmsdcc_dump_sdcc_state(host);
#ifdef  CONFIG_MMC_SD_BATTLOG_CUST_SH
            mmc_set_err_cmd_type(host->mmc, data->mrq->cmd, _DATA_TIMEOUT);
#endif
		}
#endif /* CONFIG_USE_HS200_EMMC_CUST_SH || CONFIG_USE_UHS1_SD_CUST_SH */
...
static void msmsdcc_do_cmdirq(struct msmsdcc_host *host, uint32_t status)
{
	...
	if (status & (MCI_CMDTIMEOUT | MCI_AUTOCMD19TIMEOUT)) {
		...
		cmd->error = -ETIMEDOUT;
#ifdef  CONFIG_MMC_SD_BATTLOG_CUST_SH
            mmc_set_err_cmd_type(host->mmc, cmd, _CMD_TIMEOUT);
#endif
		...
		cmd->error = -EILSEQ;
#ifdef  CONFIG_MMC_SD_BATTLOG_CUST_SH
            mmc_set_err_cmd_type(host->mmc, cmd, _CRC_ERROR);
#endif
	}
...
msmsdcc_set_enpwr_gpio(struct msmsdcc_host *host, bool enable)
{
	...
	if( enable ) {
		if (!strcmp(mmc_hostname(host->mmc),HOST_MMC_SD)) {
			...
			if (SDVDD_ON_TIME_MIN > ((end - start) / 1000000)) {
				msleep(SDVDD_ON_TIME_MIN);
			}
		}

		rc = gpio_request(sdpow_en_mpp, "sdpow_en");
		if (rc){
			pr_err("request for sdpow_en mpp failed,"
							 "rc=%d\n", rc);
		} else {
			gpio_set_value_cansleep(sdpow_en_mpp, enable);
			
			gpio_free(sdpow_en_mpp);
			msmsdcc_gpio_flg = 1;
		}
...
#ifdef CONFIG_MMC_SD_BATTLOG_CUST_SH
        mmc_detection_status_update(host->mmc, status);
#endif
...
static void msmsdcc_req_tout_timer_hdlr(unsigned long data)
{
	...
	if (mrq && mrq->cmd) {
		if (!mrq->cmd->bkops_busy) {
			...
			msmsdcc_dump_sdcc_state(host);
#ifdef  CONFIG_MMC_SD_BATTLOG_CUST_SH
            mmc_set_err_cmd_type(host->mmc, mrq->cmd, _REQ_TIMEOUT);
#endif
		}
...
{
	int *count_tmp;
	struct pm_qos_request *pm_qos_req_dma_tmp;
	...
	if (strncmp(mmc_hostname(host->mmc), HOST_MMC_MMC, sizeof(HOST_MMC_MMC)) == 0) {
	...
	if (mmc) {
		...
		if (!rc) {
			...
			if (mmc->card && mmc_card_sdio(mmc->card) &&
				...
				int err;
				err = mmc_stop_bkops(mmc->card);	
				if (err)	
					pr_warn( "%s: %d failed to stop bkops\n", mmc_hostname(mmc), err );
