...
#define DEVICE_ATTRIBUTE_SHOW_SPRINTF(A, arg...)		snprintf(A, PAGE_SIZE, ## arg)
#else
#define DEVICE_ATTRIBUTE_SHOW_SPRINTF(A, arg...)		sprintf(A, ## arg)
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "%d\n", regulator->uA_load);
}
#endif /* CONFIG_MFD_WM5102 */
#endif /* CONFIG_SH_AUDIO_DRIVER */
				...
				struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&rdev->mutex);
	ret = DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "%d\n", _regulator_get_voltage(rdev));
	mutex_unlock(&rdev->mutex);
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "%s\n", rdev_get_name(rdev));
}
...
static ssize_t regulator_print_opmode(char *buf, int mode)
{
	switch (mode) {
	case REGULATOR_MODE_FAST:
		return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "fast\n");
	case REGULATOR_MODE_NORMAL:
		return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "normal\n");
	case REGULATOR_MODE_IDLE:
		return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "idle\n");
	case REGULATOR_MODE_STANDBY:
		return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "standby\n");
	}
	return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "unknown\n");
}
...
{
	if (state > 0)
		return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "enabled\n");
	else if (state == 0)
		return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "disabled\n");
	else
		return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "unknown\n");
}
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (!rdev->constraints)
		return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "constraint not defined\n");
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case REGULATOR_VOLTAGE:
		return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "voltage\n");
	case REGULATOR_CURRENT:
		return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "current\n");
	}
	return DEVICE_ATTRIBUTE_SHOW_SPRINTF(buf, "unknown\n");
}
...
static void print_constraints(struct regulator_dev *rdev)
{
	...
	char buf[80] = "";
#ifdef CONFIG_BATTERY_SH
	const int buf_size = sizeof(buf)/sizeof(char);
#endif	/* CONFIG_BATTERY_SH */
	int count = 0;
	int ret;
	...
	if (constraints->min_uV && constraints->max_uV) {
		if (constraints->min_uV == constraints->max_uV)
#ifdef CONFIG_BATTERY_SH
			count += snprintf(buf + count,  buf_size - count, "%d mV ",
...
#ifdef CONFIG_BATTERY_SH
			count += snprintf(buf + count, buf_size - count, "%d <--> %d mV ",
					 constraints->min_uV / 1000,
	...
	if (!constraints->min_uV ||
		...
		if (ret > 0)
#ifdef CONFIG_BATTERY_SH
			count += snprintf(buf + count, buf_size - count, "at %d mV ", ret / 1000);
#else
...
#ifdef CONFIG_BATTERY_SH
		count += snprintf(buf + count, buf_size - count, "%dmV offset ",
	...
	if (constraints->min_uA && constraints->max_uA) {
		if (constraints->min_uA == constraints->max_uA)
#ifdef CONFIG_BATTERY_SH
			count += snprintf(buf + count, buf_size - count, "%d mA ",
...
#ifdef CONFIG_BATTERY_SH
			count += snprintf(buf + count, buf_size - count, "%d <--> %d mA ",
					 constraints->min_uA / 1000,
	...
	if (!constraints->min_uA ||
		...
		if (ret > 0)
#ifdef CONFIG_BATTERY_SH
			count += snprintf(buf + count, buf_size - count, "at %d mA ", ret / 1000);
#else
...
#ifdef CONFIG_BATTERY_SH
		count += snprintf(buf + count, buf_size - count, "fast ");
#else
		count += sprintf(buf + count, "fast ");
#endif	/* CONFIG_BATTERY_SH */
	if (constraints->valid_modes_mask & REGULATOR_MODE_NORMAL)
#ifdef CONFIG_BATTERY_SH
		count += snprintf(buf + count, buf_size - count, "normal ");
#else
		count += sprintf(buf + count, "normal ");
#endif	/* CONFIG_BATTERY_SH */
	if (constraints->valid_modes_mask & REGULATOR_MODE_IDLE)
#ifdef CONFIG_BATTERY_SH
		count += snprintf(buf + count, buf_size - count, "idle ");
#else
		count += sprintf(buf + count, "idle ");
#endif	/* CONFIG_BATTERY_SH */
	if (constraints->valid_modes_mask & REGULATOR_MODE_STANDBY)
#ifdef CONFIG_BATTERY_SH
		count += snprintf(buf + count, buf_size - count, "standby");
#else
					  ...
					  const char *supply_name)
{
	...
	if (dev) {
		...
		err = sysfs_create_link_nowarn(&rdev->dev.kobj, &dev->kobj, buf);
		if (err == -EEXIST) {
			/* ignore error */
			rdev_dbg(rdev, "already exists device link %s\n", dev->kobj.name);
		} else
#endif /* CONFIG_BATTERY_SH */
		if (err) {
