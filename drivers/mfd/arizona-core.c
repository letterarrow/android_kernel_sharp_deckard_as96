...
int __devinit arizona_dev_init(struct arizona *arizona)
{
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 03-104 */
	arizona->hw_rev = 0x7 & sh_boot_get_hw_revision();
	dev_info(arizona->dev, "arizona_dev_init:hw_rev %u\n", arizona->hw_rev);
	...
	if ((arizona->hw_rev == HW_REVISION_ES0) || (arizona->hw_rev == HW_REVISION_ES1)) {
		//change for MICD_LEVEL_SEL
		dev_info(arizona->dev, "set ARIZONA_MICD_LVL");
		regmap_update_bits(arizona->regmap, ARIZONA_MIC_DETECT_2, ARIZONA_MICD_LVL_SEL_MASK, 0x90);
	}
	...
	regmap_update_bits(arizona->regmap, ARIZONA_HPLPF1_2, 0xFFFF, 0xF001);
	regmap_update_bits(arizona->regmap, ARIZONA_HPLPF2_2, 0xFFFF, 0xF001);
	regmap_update_bits(arizona->regmap, ARIZONA_HPLPF3_2, 0xFFFF, 0xF001);
	regmap_update_bits(arizona->regmap, 0x179, 0xFFFF, 0x0118);
	regmap_update_bits(arizona->regmap, 0x458, 0xFFFF, 0x0000);
