...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00204 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00024 */ /* CUST_ID_00026 */ /* CUST_ID_00033 */ /* CUST_ID_00035 */ /* CUST_ID_00042 */ /* CUST_ID_00132 */
...
#define MSM_FB_BLACKSCREEN_TIMEOUT 40
#else
#define MSM_FB_BLACKSCREEN_TIMEOUT 270
#endif
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00015 */ /* CUST_ID_00038 */ /* CUST_ID_00051 */ /* CUST_ID_00058 */ /* CUST_ID_00071 */ /* CUST_ID_00084 */ /* CUST_ID_00089 */ /* CUST_ID_00094 */ /* CUST_ID_00101 */ /* CUST_ID_00132 */
int overlay_start;
int overlay_stop;
...
int fb_suspend_cond = FB_SUSPEND_COND_NORMAL_ON;
static struct work_struct msm_fb_blackscreen_worker;
static int msm_fb_blackscreen_type = 0;
static int msm_fb_blackscreen_timeout_cnt = 0;
static int msm_fb_disp_start = FALSE;
DEFINE_MUTEX(msm_fb_blackscreen_sem);
static void msm_fb_blackscreen_wq_handler(struct work_struct *work);
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00204 */
#ifdef CONFIG_USES_SHLCDC
static struct perf_lock msm_fb_perf_lock;
int msm_fb_perf_lock_on;
#endif
#endif
...
#ifdef CONFIG_SHLCDC_BOARD  /* CUST_ID_00132 */ /* CUST_ID_00204 */
        INIT_WORK(&msm_fb_blackscreen_worker, msm_fb_blackscreen_wq_handler);
#ifdef CONFIG_USES_SHLCDC
		perf_lock_init(&msm_fb_perf_lock, PERF_LOCK_486MHz, "msm_fb_perf_lock");
#endif
#endif        
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00026 */ /* CUST_ID_00071 */ /* CUST_ID_00101 */ /* CUST_ID_00143 */ /* CUST_ID_00132 */
    ...
    if ((pdata->panel_info.pdest == DISPLAY_1) && (mfd != NULL)){
        if((mfd->panel.type == MIPI_CMD_PANEL) || (mfd->panel.type == MIPI_VIDEO_PANEL)){
#ifndef CONFIG_USES_SHLCDC /* CUST_ID_00024 */
            lock_recovery();
            ...
            if(fb_suspend_cond == FB_SUSPEND_COND_1HZ){
                if(pdata->off){
#ifdef CONFIG_USES_SHLCDC /* CUST_ID_00024 */
                    lock_recovery();
#endif	/* CONFIG_USES_SHLCDC */
...
void msm_fb_blackscreen_lock_check(void)
{
    if(fb_suspend_cond != FB_SUSPEND_COND_NORMAL_ON){
        mutex_lock(&msm_fb_blackscreen_sem);
        msm_fb_blackscreen_timeout_cnt -= 30;
        mutex_unlock(&msm_fb_blackscreen_sem);
    }
}

void msm_fb_check_blackscreen_timeout(int type)
{
#if defined(CONFIG_SHDISP_PANEL_IGZO)
    msm_fb_blackscreen_type = type;
    schedule_work(&msm_fb_blackscreen_worker);
#endif
}
...
static void msm_fb_blackscreen_wq_handler(struct work_struct *work)
{
    ...
    lock_recovery();
    if(fb_suspend_cond == FB_SUSPEND_COND_1HZ){
        switch(msm_fb_blackscreen_type){
        case SHDISP_BLACKSCREEN_TYPE_T10:
            msm_fb_blackscreen_timeout_cnt += 10;
            break;
        case SHDISP_BLACKSCREEN_TYPE_T30:
            msm_fb_blackscreen_timeout_cnt += 30;
            break;
        default:
            break;
        }
        if(msm_fb_blackscreen_timeout_cnt >= MSM_FB_BLACKSCREEN_TIMEOUT){
            int idx;
            struct platform_device *pdev;
            struct msm_fb_panel_data *pdata;
            struct msm_fb_data_type *mfd;
            for(idx = 0; idx < pdev_list_cnt; idx++){
                pdev = pdev_list[idx];
                if(!pdev){
                    continue;
                }
                pdata = pdev->dev.platform_data;
                mfd = (struct msm_fb_data_type *)platform_get_drvdata(pdev);
                if(pdata && pdata->panel_info.pdest == DISPLAY_1 && pdata->off){
                    fb_suspend_cond = FB_SUSPEND_COND_OFF;
                    pdata->off(mfd->pdev);
                    fb_suspend_cond = FB_SUSPEND_COND_NORMAL_ON;
                }
...
#ifdef CONFIG_SHLCDC_BOARD  /* CUST_ID_00132 */
static int msm_fb_prepare(struct device *dev)
{
	struct msm_fb_data_type *mfd = dev_get_drvdata(dev);
	struct msm_fb_panel_data *pdata = NULL;
	int ret = 0;
...
static void msmfb_early_suspend(struct early_suspend *h)
{
    ...
    SHDISP_VIDEO_PERFORMANCE("SUSPEND PANEL EARLY-SUSPEND 0010 START\n");
	...
	*/
...
#endif
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00024 */ /* CUST_ID_00042 */ /* CUST_ID_00051 */ /* CUST_ID_00101 */ /* CUST_ID_00132 */
	if (msm_fb_shutdown_in_progress) 
		return;
	SHLCDC_EVENTLOG_REC(FB,LCD,MSM_FB_KERL,EEVENTID_FB_EARLY_SUSPEND,0);
#ifndef CONFIG_USES_SHLCDC /* CUST_ID_00024 */
	lock_recovery();
#endif /* CONFIG_USES_SHLCDC */
	msm_fb_suspend_sub(mfd);
	...
	SHDISP_VIDEO_PERFORMANCE("SUSPEND PANEL EARLY-SUSPEND 0010 END\n");
}
...
static void msmfb_early_resume(struct early_suspend *h)
{
    ...
    SHDISP_VIDEO_PERFORMANCE("RESUME PANEL EARLY-RESUME 0010 START\n");
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00024 */ /* CUST_ID_00042 */ /* CUST_ID_00097 */ /* CUST_ID_00101 */ /* CUST_ID_00132 */
	if (msm_fb_shutdown_in_progress)
		return;
	SHLCDC_EVENTLOG_REC(FB,LCD,MSM_FB_KERL,EEVENTID_FB_EARLY_RESUME,0);
#ifndef CONFIG_USES_SHLCDC /* CUST_ID_00024 */
	lock_recovery();
#endif /* CONFIG_USES_SHLCDC */
    ...
    SHDISP_VIDEO_PERFORMANCE("RESUME PANEL EARLY-RESUME 0010 END\n");
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00024 */ /* CUST_ID_00116 */ /* CUST_ID_00132 */ /* CUST_ID_00139 */
...
void msm_fb_set_backlight(struct msm_fb_data_type *mfd, __u32 bkl_lvl)
{
    ...
    SHDISP_VIDEO_PERFORMANCE("RESUME PANEL BACKLIGHT-SET 0010 START\n");
	...
	if (!mfd->panel_power_on || !bl_updated) {
        ...
        SHDISP_VIDEO_PERFORMANCE("RESUME PANEL BACKLIGHT-SET 0020 START\n");
	...
	SHDISP_VIDEO_PERFORMANCE("RESUME PANEL BACKLIGHT-SET 0010 END\n");
			    ...
			    boolean op_enable)
{
	...
	if( pdata->panel_info.pdest == DISPLAY_1 ) {
		if( fb_suspend_cond == FB_SUSPEND_COND_OFF ) {
			return 0;
		}
	}
#endif
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00048 */ /* CUST_ID_00134 */ /* CUST_ID_00159 */ /* CUST_ID_00160 */
			if(mfd->panel_info.type == MIPI_CMD_PANEL)
				mipi_sharp_delay_us(16666);
#endif	/* CONFIG_SHLCDC_BOARD */
            SHDISP_VIDEO_PERFORMANCE("RESUME MSMFB RESUME-START 0010 START\n");
...
#endif	/* CONFIG_USES_SHLCDC */
#endif	/* CONFIG_SHLCDC_BOARD */
			SHDISP_VIDEO_PERFORMANCE("RESUME MSMFB RESUME-START 0010 END\n");
		}
		break;
	...
	default:
		if (mfd->panel_power_on) {
...
#if defined(CONFIG_SHDISP_PANEL_IGZO)
			if( pdata->panel_info.pdest == DISPLAY_1 ) {
				int get_music_flg = shterm_get_music_info();
                int fw_timeout_flg = mipi_sharp_is_fw_timeout();
				int fw_recovery_failed_flg = mipi_sharp_is_recovery_faild();
				if( get_music_flg || msm_fb_shutdown_in_progress || !msm_fb_disp_start || fw_timeout_flg || fw_recovery_failed_flg) {
		            fb_suspend_cond = FB_SUSPEND_COND_NORMAL_ON;
				} else {
            ...
            SHDISP_VIDEO_PERFORMANCE("SUSPEND MSMFB SUSPEND-START 0010 START\n");
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00024 */
#ifdef CONFIG_USES_SHLCDC
			lock_recovery();
#endif	/* CONFIG_USES_SHLCDC */
			...
			if (ret) {
				...
				msm_fb_blackscreen_timeout_cnt = 0;
			}
#else
			if (ret)
				mfd->panel_power_on = curr_pwr_state;
			...
			overlay_state = 0;
#endif	/* CONFIG_SHLCDC_BOARD */
        SHDISP_VIDEO_PERFORMANCE("SUSPEND MSMFB SUSPEND-START 0010 END\n");
		}
		break;
	}
...
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00203 */
	msm_fb_pan_idle(mfd);
	if (off >= len) {
		/* memory mapped io */
		off -= len;
...
static int msm_fb_open(struct fb_info *info, int user)
{
	...
	if (mfd->op_enable == 0) {
#if CONFIG_SHLCDC_BOARD /* CUST_ID_00186 */
		if (!mfd->ref_cnt && info->node == 2) {
			/* force EPERM error when open fb2 in suspend state */
			return -EPERM;
		}
#else
		if (info->node == 2)
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00201 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00084 */ /* CUST_ID_00120 */ /* CUST_ID_00163 */ /* CUST_ID_00184 */
	if (mfd->panel_power_on && !lcdon_cmd_send) {
		if(mfd->panel.type == MIPI_VIDEO_PANEL
			|| (mfd->panel.type == MIPI_CMD_PANEL && !mdp4_dsi_cmd_overlay_get_err_flg())){
			pdata = (struct msm_fb_panel_data *)mfd->pdev->
				dev.platform_data;
			if ((pdata) && (pdata->start_display)) {
				mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
...
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00113 */
	if (info->node == 0 && (mfd->cont_splash_done)) /* primary */
...
static void msm_fb_commit_wq_handler(struct work_struct *work)
{
	...
	struct fb_info *info;
	struct msm_fb_backup_type *fb_backup;
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00185 */
	struct msm_fb_panel_data *pdata;
#endif
	...
	if (fb_backup->disp_commit.flags &
		MDP_DISPLAY_COMMIT_OVERLAY) {
			SHDISP_VIDEO_PERFORMANCE("RESUME PANEL OVERLAY-COMMIT 0010 START\n");
			mdp4_overlay_commit(info);
			SHDISP_VIDEO_PERFORMANCE("RESUME PANEL OVERLAY-COMMIT 0010 END\n");
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00156 */ /* CUST_ID_00163 */ /* CUST_ID_00184 */ /* CUST_ID_00185 */
		if (mfd->panel_power_on && !lcdon_cmd_send) {
			if(mfd->panel.type == MIPI_VIDEO_PANEL
				|| (mfd->panel.type == MIPI_CMD_PANEL && !mdp4_dsi_cmd_overlay_get_err_flg())){
				pdata = (struct msm_fb_panel_data *)mfd->pdev->dev.platform_data;
				...
				SHDISP_VIDEO_PERFORMANCE("RESUME PANEL DISP-INIT 0010 START\n");
				shdisp_api_main_disp_init_2nd();
				SHDISP_VIDEO_PERFORMANCE("RESUME PANEL DISP-INIT 0010 END\n");
				SHDISP_VIDEO_PERFORMANCE("RESUME PANEL DISP-ON 0010 START\n");
				shdisp_api_main_disp_on();
				SHDISP_VIDEO_PERFORMANCE("RESUME PANEL DISP-ON 0010 END\n");
				msm_lcd_recovery_subscribe();
			}
		}

		if (unset_bl_level && !bl_updated && lcdon_cmd_send ) {
			pdata = (struct msm_fb_panel_data *)mfd->pdev->dev.platform_data;
			if ((pdata) && (pdata->set_backlight)) {
				down(&mfd->sem);
				mfd->bl_level = unset_bl_level;
...
static int msmfb_overlay_play(struct fb_info *info, unsigned long *argp)
{
	...
	struct msmfb_overlay_data req;
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
#ifndef CONFIG_SHLCDC_BOARD  /* CUST_ID_00185 */
	struct msm_fb_panel_data *pdata;
#endif
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00204 */
#ifdef CONFIG_USES_SHLCDC
DEFINE_MUTEX(msm_fb_perf_lock_sem);
#endif
#endif
...
static int msmfb_notify_update(struct fb_info *info, unsigned long *argp)
{
	...
	if (notify == NOTIFY_UPDATE_START) {
		INIT_COMPLETION(mfd->msmfb_update_notify);
#if defined(CONFIG_SHLCDC_BOARD) && defined(CONFIG_SHDISP_PANEL_RYOMA)  /* CUST_ID_00140 */
			...
			unsigned long arg)
{
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00006 */ /* CUST_ID_00007 */ /* CUST_ID_00008 */ /* CUST_ID_00010 */ /* CUST_ID_00011 */ /* CUST_ID_00012 */ /* CUST_ID_00013 */ /* CUST_ID_00015 */ /* CUST_ID_00021 */ /* CUST_ID_00038 */ /* CUST_ID_00044 */ /* CUST_ID_00051 */ /* CUST_ID_00058 */ /* CUST_ID_00085 */ /* CUST_ID_00089 */ /* CUST_ID_00090 */ /* CUST_ID_00XXX */ /* CUST_ID_00169 */
	case MSMFB_SET_PIPE_HIST_LUT:
		...
		down(&msm_fb_ioctl_ppp_sem);
		ret = mdp4_vg_hist_lut_update(&pipe_hist_lut_data, 1);
		up(&msm_fb_ioctl_ppp_sem);
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00169 */
int msm_fb_set_pipe_hist_lut( struct fb_info *info, struct msmfb_pipe_hist_lut_data *hist_lut )
{
	struct msm_fb_data_type *mfd = NULL;
	int ret;

	/* parameter check */
	if( !info || !info->par || !hist_lut )
		return -EINVAL;

	mfd = (struct msm_fb_data_type *)info->par;
	memcpy(&pipe_hist_lut_data, hist_lut, sizeof(pipe_hist_lut_data));

	msm_fb_pan_idle(mfd);

	down(&msm_fb_ioctl_ppp_sem);
	ret = mdp4_vg_hist_lut_update(&pipe_hist_lut_data, 0);
	up(&msm_fb_ioctl_ppp_sem);

	return ret;
}
EXPORT_SYMBOL(msm_fb_set_pipe_hist_lut);

int msm_fb_set_pipe_qseed( struct fb_info *info, struct msmfb_pipe_qseed_data *qseed )
{
	struct msm_fb_data_type *mfd = NULL;
	int ret;

	/* parameter check */
	if( !info || !info->par || !qseed )
		return -EINVAL;

	mfd = (struct msm_fb_data_type *)info->par;
	memcpy(&pipe_qseed_data, qseed, sizeof(pipe_qseed_data));
...
EXPORT_SYMBOL(msm_fb_set_pipe_qseed);

int msm_fb_set_ccs_matrix( struct fb_info *info, struct mdp_csc *csc_matrix )
{
	struct msm_fb_data_type *mfd = NULL;

	/* parameter check */
	if( !info || !info->par || !csc_matrix )
		return -EINVAL;
...
EXPORT_SYMBOL(msm_fb_set_ccs_matrix);
#endif /* CONFIG_SHLCDC_BOARD */
...
{
	mutex_lock(&msm_fb_perf_lock_sem);

	if( on ) {
		if (! msm_fb_perf_lock_on){
			perf_lock(&msm_fb_perf_lock);
			msm_fb_perf_lock_on = 1;
		}
	} else {
		if (msm_fb_perf_lock_on){
			perf_unlock(&msm_fb_perf_lock);
			msm_fb_perf_lock_on = 0;
		}
	}

	mutex_unlock(&msm_fb_perf_lock_sem);
}
#endif
#endif
