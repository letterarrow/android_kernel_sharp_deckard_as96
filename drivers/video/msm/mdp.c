...
#include <mach/clk.h>
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00173 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00173 */
static struct perf_lock mdp_perf_lock;
static int perflock_enable=0;
module_param(perflock_enable, int, 0600);
#endif

#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00132 */
static void mdp_early_suspend(struct early_suspend *h);
static void mdp_early_resume(struct early_suspend *h);
static void mdp_early_resume_sub(struct early_suspend *h);
#endif
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00014 */ /* CUST_ID_00022 */ /* CUST_ID_00037 */ /* CUST_ID_00038 */ /* CUST_ID_00058 */ /* CUST_ID_00083 */ /* CUST_ID_00XXX */
static int mdp_dma_abl_lut_set(struct fb_cmap *cmap)
{
	int ret, len;
...
static int mdp_dma_abl_lut_read_diagarea(void)
{
	...
	if( p == NULL) {
		...
		for (i = 0; i < MDP_ABL_LUT_SIZE; i++) {
#ifdef CONFIG_USES_SHLCDC
			lut_abl_gamma[i].red   = (uint16_t)i;
			lut_abl_gamma[i].green = (uint16_t)i;
			lut_abl_gamma[i].blue  = (uint16_t)i;
#else
...
{
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00014 */ /* CUST_ID_00058 */ /* CUST_ID_00XXX */
...
static int mdp_off(struct platform_device *pdev)
{
    ...
    if(fb_suspend_cond == FB_SUSPEND_COND_OFF){
        struct early_suspend *tmp = NULL;
        mdp_early_resume_sub(tmp);
...
#endif
        mdp_early_suspend(tmp);
	{
		extern int rpm_regulator_tcxo_wakelock_active(void);
		int count = 5;
		while( rpm_regulator_tcxo_wakelock_active() != 0 && count > 0 ){
			signed long to = msecs_to_jiffies(4)+1;
			while( to > 0 ){
    ...
    SHDISP_VIDEO_PERFORMANCE("SUSPEND MDP DISP-OFF 0010 START\n");
    shdisp_api_main_disp_off();
    SHDISP_VIDEO_PERFORMANCE("SUSPEND MDP DISP-OFF 0010 END\n");
...
static int mdp_on(struct platform_device *pdev)
{
...
#ifdef CONFIG_SHLCDC_BOARD  /* CUST_ID_00204 */ 
#ifdef CONFIG_USES_SHLCDC
	msm_fb_perflock_ctrl(1);
#endif
#endif
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00020 */ /* CUST_ID_00041 */ /* CUST_ID_00084 */ /* CUST_ID_00132 */
    if(fb_suspend_cond == FB_SUSPEND_COND_NORMAL_ON){
		SHDISP_VIDEO_PERFORMANCE("RESUME MDP POWER-ON 0010 START\n");
	    shdisp_api_main_lcd_power_on();
		SHDISP_VIDEO_PERFORMANCE("RESUME MDP POWER-ON 0010 END\n");
    }
	SHDISP_VIDEO_PERFORMANCE("RESUME MDP DISP-INIT 0010 START\n");
	shdisp_api_main_disp_init_1st();
	SHDISP_VIDEO_PERFORMANCE("RESUME MDP DISP-INIT 0010 END\n");
	...
	if (mdp_rev >= MDP_REV_40) {
		...
		if (mfd->panel.type == MIPI_CMD_PANEL) {
#ifdef CONFIG_SHLCDC_BOARD  /* CUST_ID_00111 */
			if (!(mfd->cont_splash_done)) {
				mfd->cont_splash_done = 1;
			}
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00173 */
		perf_lock_init(&mdp_perf_lock, PERF_LOCK_594MHz, "mdp_perf_lock_low");
#endif
...
void mdp_perflock_ctrl(int on)
{
	...
	if( on ) {
		perf_lock(&mdp_perf_lock);
	} else {
		perf_unlock(&mdp_perf_lock);
	}
}
#endif
...
static void mdp_early_suspend(struct early_suspend *h)
{
    ...
    SHDISP_VIDEO_PERFORMANCE("SUSPEND MDP EARLY-SUSPEND 0010 START\n");

#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00101 */ /* CUST_ID_00132 */
	if (msm_fb_shutdown_in_progress && fb_suspend_cond != FB_SUSPEND_COND_OFF)
		return;
#endif /* CONFIG_SHLCDC_BOARD */
	mdp_suspend_sub();
#ifdef CONFIG_FB_MSM_DTV
	...
	SHDISP_VIDEO_PERFORMANCE("SUSPEND MDP EARLY-SUSPEND 0010 END\n");
...
static void mdp_early_resume_sub(struct early_suspend *h)
{
    SHDISP_VIDEO_PERFORMANCE("RESUME MDP EARLY-RESUME 0010 START\n");
	...
	SHDISP_VIDEO_PERFORMANCE("RESUME MDP EARLY-RESUME 0010 END\n");
}

static void mdp_early_resume(struct early_suspend *h)
{
    msm_fb_blackscreen_lock_check();
    mdp_early_resume_sub(h);
}
