...
#include <linux/fb.h>
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00180 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00051 */ /* CUST_ID_00051 */ /* CUST_ID_00089 */ /* CUST_ID_00181 */
#include "mipi_sharp.h"
extern int fps_low_mode;
extern int base_fps_low_mode;
extern void mdp_req_kickoff_pipe_debug(int flag);
extern void mdp4_overlay_mdp_bw_reset(struct msm_fb_data_type *mfd);
extern void mdp_perflock_ctrl(int on);
extern void mdp4_set_writeback_buf(struct msm_fb_data_type *, u32, int);
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00181 */
	mdp4_set_writeback_buf(mfd, MDP4_MIXER0, pipe->ov_blt_addr);
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00176 */
	mdp4_dtv_wait_unset_dmae();
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00182 */ /* FIXME */
	if (wait)
...
void mdp4_dsi_cmd_vsync_ctrl(struct fb_info *info, int enable)
{
    ...
    if (atomic_read(&vctrl->suspend) > 0 && enable){
        return;
    }
#endif
	...
	if (enable) {
		...
		if (clk_set_on) {
			...
			pr_debug("LCDDBG: [%s] gpio(%d) set value at 1\n", __func__, PM8921_GPIO_PM_TO_SYS(15));
			gpio_set_value(PM8921_GPIO_PM_TO_SYS(15), 1); /* PLL enable */
...
static void clk_ctrl_work(struct work_struct *work)
{
	...
	if (vctrl->clk_control && vctrl->clk_enabled) {
		...
		mipi_dsi_clk_cfg(0);
		mdp_clk_ctrl(0);
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00173 */ /* CUST_ID_00179 */ /* CUST_ID_00180 */
		mdp4_overlay_mdp_bw_reset(vctrl->mfd);
		mdp_perflock_ctrl(0);
#ifdef CONFIG_USES_SHLCDC
		pr_debug("LCDDBG: [%s] gpio(%d) set value at 0\n", __func__, PM8921_GPIO_PM_TO_SYS(15));
		gpio_set_value(PM8921_GPIO_PM_TO_SYS(15), 0); /* PLL disable */
#endif
#endif
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00122 */ /* CUST_ID_00167 */
	MDP_OUTP(MDP_BASE + 0x021c, ((mfd->total_lcd_lines - 1) - vsync_start_y_adjust + 14) % (mfd->total_lcd_lines - 1));
#else
...
int mdp4_dsi_cmd_off(struct platform_device *pdev)
{
	...
	complete_all(&vctrl->vsync_comp);
#ifdef CONFIG_SHLCDC_BOARD  /* CUST_ID_00107 */
	complete_all(&vctrl->show_comp);	/* CUST_ID_00107 */
#endif  /* CUST_ID_00107 */
	...
	if (cnt > 10) {
		spin_lock_irqsave(&vctrl->spin_lock, flags);
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00183 */
		vsync_irq_disable(INTR_PRIMARY_RDPTR, MDP_PRIM_RDPTR_TERM);
		vctrl->vsync_enabled = 0;
#endif
		vctrl->clk_control = 0;
	...
	if (pipe) {
		if(!mipi_sharp_is_recovery()){
			...
			mdp4_overlay_unsecure(mfd, pipe->mixer_num);
#endif /* CONFIG_SHLCDC_BOARD */
...
static int mdp4_dsi_cmd_clk_check(struct vsycn_ctrl *vctrl)
{
	...
	if (clk_set_on) {
		...
		mipi_dsi_clk_cfg(1);
		mdp_clk_ctrl(1);
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00173 */ /* CUST_ID_00179 */
		mdp_perflock_ctrl(1);
#ifdef CONFIG_USES_SHLCDC
		pr_debug("LCDDBG: [%s] gpio(%d) set value at 1\n", __func__, PM8921_GPIO_PM_TO_SYS(15));
		gpio_set_value(PM8921_GPIO_PM_TO_SYS(15), 1); /* PLL enable */
