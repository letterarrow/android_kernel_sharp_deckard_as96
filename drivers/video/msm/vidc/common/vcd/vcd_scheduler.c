...
u32 vcd_sched_update_config(struct vcd_clnt_ctxt *cctxt)
{
	...
	if (!cctxt || !cctxt->sched_clnt_hdl) {
		...
		pr_debug("%s DEBUG division parameter: round_perfrm=%d fps_numerator=%d fps_denominator=%d\n",
			__func__, cctxt->sched_clnt_hdl->round_perfrm, cctxt->frm_rate.fps_numerator, cctxt->frm_rate.fps_denominator);
		if(cctxt->sched_clnt_hdl->round_perfrm == 0){
			VCD_MSG_ERROR("%s round_perfrm=0 to 120\n", __func__);
			cctxt->sched_clnt_hdl->round_perfrm = 120;
		}
