...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00176 */
	atomic_set(&vctrl->is_unset_dmaeseq,0);
#endif	/* CONFIG_SHLCDC_BOARD */
	return cnt;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = wait_for_completion_interruptible_timeout(&vctrl->vsync_comp,
		msecs_to_jiffies(VSYNC_PERIOD * 3 * 2));
#else
	ret = wait_for_completion_interruptible_timeout(&vctrl->vsync_comp,
...
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00162 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00201 */
		mixer = pipe->mixer_num;
		mdp4_overlay_unsecure(mfd, mixer);
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00162 */
	atomic_set(&vctrl->suspend, 1);
#endif
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00176 */
	if(vctrl->base_pipe != NULL) {
		atomic_set(&vctrl->is_unset_dmaeseq, 1);
	}
#endif	/* CONFIG_SHLCDC_BOARD */
...
void mdp4_dtv_wait_unset_dmae(void)
{
	...
	int isunsetdmaseq;
	isunsetdmaseq = atomic_read(&vctrl->is_unset_dmaeseq);
	
	if( isunsetdmaseq ){
		int ret;
		if (atomic_read(&vctrl->suspend) > 0){
			atomic_set(&vctrl->is_unset_dmaeseq, 0);
			printk("mdp4_dtv_wait_unset_dmae() no wait(supend)end.\n" );
			return;
		}
		ret = wait_for_completion_timeout(&vctrl->dmae_comp, msecs_to_jiffies(100));
		if (ret <= 0) {
			atomic_set(&vctrl->is_unset_dmaeseq, 0);
			pr_err("%s: timeout\n", __func__ );
		}
