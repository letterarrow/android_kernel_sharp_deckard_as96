...
#ifdef	CONFIG_SHLCDC_BOARD	/* CUST_ID_00189 */
#define VGPIPEIDX(param)    ((param == OVERLAY_PIPE_VG1) ? 1 : 2)
#endif	/* CUST_ID_00189 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00149 */
	if (pipe->op_mode & MDP4_OP_FLIP_LR) {
		pipe->op_mode |= MDP4_OP_SCALEX_EN;
	}

	if (pipe->op_mode & MDP4_OP_FLIP_UD) {
		pipe->op_mode |= MDP4_OP_SCALEY_EN;
	}
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00178 */
	if (req->dst_rect.h == 1) {
		pr_err("%s: dst_h is minimum range: 0X%x!\n",
		       __func__, req->dst_rect.h);
		mdp4_stat.err_size++;
		return -ERANGE;
	}
	if (req->dst_rect.w == 1) {
		pr_err("%s: dst_w is minimum range: 0X%x!\n",
		       __func__, req->dst_rect.w);
		mdp4_stat.err_size++;
		return -ERANGE;
	}
#endif /* CONFIG_SHLCDC_BOARD */ /* CUST_ID_XXXXX */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00177 */
	/* set the clock of mmfab more than 96MHz */
#if defined(MDP4_AB_BW_LOWER_LIMIT) && defined(MDP4_IB_BW_LOWER_LIMIT)
	if( perf_req->mdp_ab_bw < MDP4_AB_BW_LOWER_LIMIT ) {
		pr_debug("%s: perf_req->mdp_ab_bw force changed %u to %u\n", __func__, perf_req->mdp_ab_bw, MDP4_AB_BW_LOWER_LIMIT);
		perf_req->mdp_ab_bw = MDP4_AB_BW_LOWER_LIMIT;
	}

	if( perf_req->mdp_ib_bw < MDP4_IB_BW_LOWER_LIMIT ) {
		pr_debug("%s: perf_req->mdp_ib_bw force changed %u to %u\n", __func__, perf_req->mdp_ib_bw, MDP4_IB_BW_LOWER_LIMIT);
		perf_req->mdp_ib_bw = MDP4_IB_BW_LOWER_LIMIT;
	}
#endif
#endif
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00200 */
...
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00174 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00179 */
void mdp4_overlay_mdp_bw_reset(struct msm_fb_data_type *mfd)
{
	struct mdp4_overlay_perf *perf_cur = &perf_current;

	if( ctrl->stage[MDP4_MIXER1][MDP4_MIXER_STAGE_BASE] 
			|| ctrl->stage[MDP4_MIXER2][MDP4_MIXER_STAGE_BASE] ) {
		pr_debug("%s: mdp mixer1 or 2 still staged (mixer1=%p, mixer2=%p)\n",
			__func__, 
			ctrl->stage[MDP4_MIXER1][MDP4_MIXER_STAGE_BASE],
			ctrl->stage[MDP4_MIXER2][MDP4_MIXER_STAGE_BASE]);
		return;
	}

	pr_debug("%s: reset mdp bus scale\n", __func__);
	mdp_bus_scale_update_request(0, 0);
	perf_cur->mdp_ab_bw = 0;
	perf_cur->mdp_ib_bw = 0;
}
#endif
...
int mdp4_overlay_set(struct fb_info *info, struct mdp_overlay *req)
{
	...
	if (ret < 0) {
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00174 */
...
void mdp4_overlay_unsecure(struct msm_fb_data_type *mfd, int mixer)
{
	struct mdp4_overlay_pipe *pipe;
	int i = 0;
	...
	for (i = MDP4_MIXER_STAGE3; i > MDP4_MIXER_STAGE_BASE; i--) {
        ...
        pr_debug("%s: pipe=0x%p, pipe num=%d, sec_ov_session=%d.\n", __func__, pipe, pipe->pipe_num, (pipe->flags & MDP_SECURE_OVERLAY_SESSION));
        if (pipe->flags & MDP_SECURE_OVERLAY_SESSION) {
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00007 */ /* CUST_ID_00008 */ /* CUST_ID_00009 */ /* CUST_ID_00029 */ /* CUST_ID_00189 */
	if (pipe->pipe_num <= OVERLAY_PIPE_VG2) {
		if ((mfd->panel_info.pdest == DISPLAY_1)
				&& (mdp4_overlay_format2type(pipe->src_format) == OVERLAY_TYPE_VIDEO)) {
			if (!vg_dither_set_flg) {
				mdp4_vg_dither_setup(pipe->pipe_num);
				vg_dither_set_flg = VGPIPEIDX(pipe->pipe_num);
			}
			if (!vg_hist_lut_set_flg) {
				mdp4_vg_hist_lut_setup(pipe->pipe_num);
				vg_hist_lut_set_flg = VGPIPEIDX(pipe->pipe_num);
			}
			if (!vg_qseed_set_flg) {
				mdp4_vg_qseed_setup();
				vg_qseed_set_flg = VGPIPEIDX(pipe->pipe_num);
			}
			if (!vg_csc_set_flg) {
				mdp4_vg_csc_setup(pipe->pipe_num);
				vg_csc_set_flg = VGPIPEIDX(pipe->pipe_num);
			}
		}
		if (mdp4_overlay_format2type(pipe->src_format) != OVERLAY_TYPE_VIDEO) {
			if (vg_dither_set_flg == VGPIPEIDX(pipe->pipe_num)) {
				mdp4_vg_dither_disable(pipe->pipe_num);
				vg_dither_set_flg = 0;
			}
			if (vg_hist_lut_set_flg == VGPIPEIDX(pipe->pipe_num)) {
				mdp4_vg_hist_lut_disable(pipe->pipe_num);
				vg_hist_lut_set_flg = 0;
			}
			if (vg_qseed_set_flg == VGPIPEIDX(pipe->pipe_num)) {
				mdp4_vg_qseed_update_init();
				mdp4_vg_qseed_setup();
				vg_qseed_set_flg = 0;
			}
			if (vg_csc_set_flg == VGPIPEIDX(pipe->pipe_num)) {
				vg_csc_set_flg = 0;
			}
		}
	}
