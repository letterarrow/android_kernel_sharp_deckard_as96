...
void mdp4_hw_init(void)
{
	ulong bits;
	uint32 clk_rate;
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_XXXXX */
	int i;
#endif
	/* MDP cmd block enable */
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
...
#ifdef CONFIG_SHLCDC_BOARD  /* CUST_ID_XXXXX */
	mdp4_vg_csc_setup(0);
	mdp4_vg_csc_setup(1);
...
static unsigned int wb_mem_hid[2] = {0};
static unsigned int wb_mem_hid_used[2] = {0};
static void update_wb_mem_hid(struct msm_fb_data_type *mfd, u32 mix_num, int flag)
{
	if (flag == 1) {
		unsigned int mem_hid = mfd->mem_hid;
		if (mfd->sec_mapped) {
			mem_hid &= ~BIT(ION_IOMMU_HEAP_ID);
			mem_hid |= ION_SECURE;
		} else {
			mem_hid |= BIT(ION_IOMMU_HEAP_ID);
			mem_hid &= ~ION_SECURE;
		}
		if (wb_mem_hid[mix_num] != mem_hid) {
			if (wb_mem_hid[mix_num] && !wb_mem_hid_used[mix_num]) {
				/* switch from/to secure mode to/from unsecure mode */
				mdp4_free_writeback_buf(mfd, mix_num);
			}
			wb_mem_hid[mix_num] = mem_hid;
		}
	} else {
		wb_mem_hid[mix_num] = 0;
	}
	return;
}

void mdp4_set_writeback_buf(struct msm_fb_data_type *mfd, u32 mix_num, int used)
{
	wb_mem_hid_used[mix_num] = used;
	return;
}
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00181 */
	update_wb_mem_hid(mfd, mix_num, 1);
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00181 */
		pr_info("%s:%d ion based allocation ion_heap 0x%x ion_flag 0x%x\n",
			__func__, __LINE__, wb_mem_hid[mix_num], wb_mem_hid[mix_num] & ION_SECURE);
		buf->ihdl = ion_alloc(mfd->iclient, buffer_size, SZ_4K,
			wb_mem_hid[mix_num], wb_mem_hid[mix_num] & ION_SECURE);
#else
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00181 */
				if (wb_mem_hid[mix_num] & ION_SECURE) {
#else
...
void mdp4_free_writeback_buf(struct msm_fb_data_type *mfd, u32 mix_num)
{
	...
	buf->read_addr = 0;
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00181 */
	update_wb_mem_hid(mfd, mix_num, 0);
#endif /* CONFIG_SHLCDC_BOARD */
}
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00007 */ /* CUST_ID_00008 */ /* CUST_ID_00010 */ /* CUST_ID_00011 */ /* CUST_ID_00012 */ /* CUST_ID_00013 */ /* CUST_ID_00022 */ /* CUST_ID_00029 */ /* CUST_ID_00044 */ /* CUST_ID_00083 */ /* CUST_ID_00XXX */ /* CUST_ID_00169 */
...
int mdp4_vg_hist_lut_update(struct msmfb_pipe_hist_lut_data *p, int user)
{
	...
	if( user ) {
		...
		for (i = 0; i < MDP_PIPE_HIST_LUT_SIZE; i++) {
			memcpy(&pipe_hist_lut_tbl[i], p->color0++, sizeof(uint8));
		}
	}
