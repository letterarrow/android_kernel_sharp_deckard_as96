...
extern int slim_change_val_elements(struct slim_device *sb, const void *r, size_t reg_size,
...
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER)
/*
 * slim_force_max_control_rate
 * Set highest clock gear and schedule the superframe fully with info
 */
extern int slim_force_max_control_rate(struct slim_device *sb, u32 *prev_clkgear);
/* 
 * slim_restore_control_rate
 * Restore control rate and subframe mode for any callers that may rely on the previously set value
 */
extern int slim_restore_control_rate(struct slim_device *sb, u32 clkgear);
#endif /* CONFIG_MFD_WM5102 && CONFIG_SH_AUDIO_DRIVER */
