...
struct tspdrv_autocal_data {
	unsigned char acalcomp;
	unsigned char acalbemf;
	unsigned char bemfgain;
};

struct tspdrv_autocal_presetting_data {
	unsigned char ratedvoltage;
	unsigned char odclamp;
	unsigned char fbctrl;
	unsigned char acal_time;
};
...
#ifdef CONFIG_SCTSPDRV
#define TSPDRV_GET_DEVICE_ID                _IOR(TSPDRV_IOCTL_GROUP, 9,unsigned char)
#define TSPDRV_AUTOCAL_EXEC                 _IOW(TSPDRV_IOCTL_GROUP,10,struct tspdrv_autocal_presetting_data)
#define TSPDRV_AUTOCAL_DATA_GET             _IOR(TSPDRV_IOCTL_GROUP,11,struct tspdrv_autocal_data)
#define TSPDRV_AUTOCAL_DATA_SET             _IO(TSPDRV_IOCTL_GROUP,12)
#endif /* CONFIG_SCTSPDRV */
