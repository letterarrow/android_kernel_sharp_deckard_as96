...
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER) /*03-001*/
static void dapm_clear_walk(struct snd_soc_dapm_context *dapm,
				   ...
				   struct list_head *source)
{
	...
	if (sink) {
		list_for_each_entry(p, sink, list_source) {
			if (p->walked) {
				p->walked = 0;
				dapm_clear_walk(NULL, &p->sink->sinks, NULL);
			}
		}
		return;
	}
	...
	if (source) {
		list_for_each_entry(p, source, list_sink) {
			if (p->walked) {
				p->walked = 0;
				dapm_clear_walk(NULL, NULL, &p->source->sources);
			}
		}
		return;
	}
...
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER) /*03-001*/
	dapm_clear_walk(dapm, &root->sinks, NULL);
#endif /* CONFIG_MFD_WM5102 && CONFIG_SH_AUDIO_DRIVER */ /*03-001*/
...
#if defined(CONFIG_MFD_WM5102) && defined(CONFIG_SH_AUDIO_DRIVER) /*03-001*/
	dapm_clear_walk(dapm, NULL, &root->sources);
#endif /* CONFIG_MFD_WM5102 && CONFIG_SH_AUDIO_DRIVER */ /*03-001*/
...
static int dapm_generic_check_power(struct snd_soc_dapm_widget *w)
{
	...
	dapm_clear_walk(w->dapm, NULL, &w->sources);
	out = is_connected_output_ep(w, NULL);
	dapm_clear_walk(w->dapm, &w->sinks, NULL);
#else  /* CONFIG_MFD_WM5102 && CONFIG_SH_AUDIO_DRIVER */ /*03-001*/
	in = is_connected_input_ep(w, NULL);
