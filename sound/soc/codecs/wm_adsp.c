...
extern struct slim_device *g_wm_slim_handler;
		   ...
		   struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		/* Force the clock gear to the highest possible fo the firmware
		 * load and coefficient download then restore to preserve the
		 * clock gearing for callers relying on the previously set value
		 */
		ret = slim_force_max_control_rate(g_wm_slim_handler, &prev_clkgear);
		if (ret)
			adsp_err(dsp, "Failed to force max control rate slim: %d\n", ret);
		...
		ret = slim_restore_control_rate(g_wm_slim_handler, prev_clkgear);
		if (ret)
			adsp_err(dsp, "Failed to restore control rate: %d\n clkgear: %d\n", ret, prev_clkgear);
