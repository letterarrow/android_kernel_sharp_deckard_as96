...
#define ARIZONA_DEBUGMODE_MSK       0x01
#define ARIZONA_DEBUGMODE_ON        0x01
#define ARIZONA_DEBUGMODE_OFF       0x00
#define ARIZONA_MICBIAS_MSK         0x02
#define ARIZONA_MICBIAS_ON          0x02
#define ARIZONA_MICBIAS_OFF         0x00
#define ARIZONA_CODECSTOP_MSK       0x04
#define ARIZONA_CODECSTOP_ON        0x04  
#define ARIZONA_CODECSTOP_OFF       0x00 

static int bias_mode_for_testmode = 0;

int arizona_codec_get_bias_mode(void)
{
	return bias_mode_for_testmode;
}
EXPORT_SYMBOL_GPL(arizona_codec_get_bias_mode);

void arizona_codec_set_bias_mode(int mode)
{
	bias_mode_for_testmode = mode;
	return;
}
EXPORT_SYMBOL_GPL(arizona_codec_set_bias_mode);
		      ...
		      struct snd_kcontrol *kcontrol, int event)
{
	...
	short reg = w->reg;
	unsigned char shift = w->shift;

	pr_debug("micbias_ev start reg=%d, shift=%x mode=%d\n",
              reg, shift, arizona_codec_get_bias_mode());

	if ((arizona_codec_get_bias_mode() & ARIZONA_DEBUGMODE_MSK) == ARIZONA_DEBUGMODE_ON) 
		regmap_update_bits(regmap, ARIZONA_MICD_CLAMP_CONTROL, 0xFF, 0x00);

	if ( ((arizona_codec_get_bias_mode() & ARIZONA_MICBIAS_MSK) == ARIZONA_MICBIAS_OFF)
	       && ((arizona_codec_get_bias_mode() & ARIZONA_DEBUGMODE_MSK) == ARIZONA_DEBUGMODE_ON) ) {
		switch (event) {
		case SND_SOC_DAPM_POST_PMU:
			pr_debug("micbias_ev SND_SOC_DAPM_POST_PMU no micbias \n");
			regmap_update_bits(regmap, reg, 0x01, 0x00);
			break;
		default:
			pr_err("micbias_ev unknown event\n");
			break;
		}
	}
	return 0;
...
/* BOUGE!! tweaking to enforce ucm config work */
static DECLARE_TLV_DB_SCALE(digital_tlv, 0, 1, 0);
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 03-006 */
static int wm5102_fmtx_mode = 0;

int arizona_codec_get_wm5102_fmtx_mode(void)
{
	return wm5102_fmtx_mode;
}
EXPORT_SYMBOL_GPL(arizona_codec_get_wm5102_fmtx_mode);

static const char *wm5102_fmtx_switch_text[] = {"ON", "OFF"};
static const struct soc_enum wm5102_fmtx_switch_enum[] = {
		SOC_ENUM_SINGLE_EXT(2, wm5102_fmtx_switch_text),
};
#define PORT_FMTX_SWITCH_GPIO PM8921_GPIO_PM_TO_SYS(44)

void wm5102_fmtx_switch_gpio_write(int param)
{
	struct pm_gpio switch_param = {
	...
	static int wm5120_fmtx_switch_param = 0;

	int ret;

	if(wm5120_fmtx_switch_param == param) {
		pr_err("%s:writed param ERR %d\n", __func__,
		    param);
		return;
	}

	ret = gpio_request(PORT_FMTX_SWITCH_GPIO, "shfmtxswitch");
	if (ret != 0) {
		pr_err("%s: Failed to request gpio shfmtxswitch %d\n", __func__,
		        PORT_FMTX_SWITCH_GPIO);
	}

	ret = pm8xxx_gpio_config(PORT_FMTX_SWITCH_GPIO, &switch_param);

	if (ret != 0) {
		pr_err("%s: Failed to configure gpio shfmtxswitch %d\n", __func__,
		        PORT_FMTX_SWITCH_GPIO);
	} else {
		gpio_direction_output(PORT_FMTX_SWITCH_GPIO, param);
		wm5120_fmtx_switch_param = param;
		wm5102_fmtx_mode = wm5120_fmtx_switch_param;
	}

	gpio_free(PORT_FMTX_SWITCH_GPIO);
}

static int wm5102_fmtx_switch_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int wm5102_fmtx_switch_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: switch put param %ld\n", __func__,
            ucontrol->value.integer.value[0]);

	switch (ucontrol->value.integer.value[0])
	{
	case 0:
		wm5102_fmtx_switch_gpio_write(1);
		return 0;

	case 1:
		wm5102_fmtx_switch_gpio_write(0);
		return 0;
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 03-006 */
SOC_ENUM_EXT("FMTX SWITCH", wm5102_fmtx_switch_enum[0],
		wm5102_fmtx_switch_get, wm5102_fmtx_switch_put),
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 03-006 */
};
...
SND_SOC_DAPM_AIF_OUT_E("SLIMTX1", "Slimbus Capture 1", 0,
		      ARIZONA_SLIMBUS_TX_CHANNEL_ENABLE,
		      ARIZONA_SLIMTX1_ENA_SHIFT, 0,
		      arizona_slim_tx_ev,
		      SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
SND_SOC_DAPM_AIF_OUT_E("SLIMTX2", "Slimbus Capture 1", 0,
		    ARIZONA_SLIMBUS_TX_CHANNEL_ENABLE,
		    ARIZONA_SLIMTX2_ENA_SHIFT, 0,
		      arizona_slim_tx_ev,
		      SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
SND_SOC_DAPM_AIF_OUT_E("SLIMTX3", "Slimbus Capture 1", 0,
		    ARIZONA_SLIMBUS_TX_CHANNEL_ENABLE,
		    ARIZONA_SLIMTX3_ENA_SHIFT, 0,
		      arizona_slim_tx_ev,
		      SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
SND_SOC_DAPM_AIF_OUT_E("SLIMTX4", "Slimbus Capture 1", 0,
		    ARIZONA_SLIMBUS_TX_CHANNEL_ENABLE,
		    ARIZONA_SLIMTX4_ENA_SHIFT, 0,
		      arizona_slim_tx_ev,
		      SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
SND_SOC_DAPM_AIF_OUT_E("SLIMTX5", "Slimbus Capture 2", 0,
		    ARIZONA_SLIMBUS_TX_CHANNEL_ENABLE,
		    ARIZONA_SLIMTX5_ENA_SHIFT, 0,
		      arizona_slim_tx_ev,
		      SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
SND_SOC_DAPM_AIF_OUT_E("SLIMTX6", "Slimbus Capture 2", 0,
		    ARIZONA_SLIMBUS_TX_CHANNEL_ENABLE,
		    ARIZONA_SLIMTX6_ENA_SHIFT, 0,
		      arizona_slim_tx_ev,
		      SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
SND_SOC_DAPM_AIF_OUT_E("SLIMTX7", "Slimbus Capture 3", 0,
		    ARIZONA_SLIMBUS_TX_CHANNEL_ENABLE,
		    ARIZONA_SLIMTX7_ENA_SHIFT, 0,
		      arizona_slim_tx_ev,
		      SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
SND_SOC_DAPM_AIF_OUT_E("SLIMTX8", "Slimbus Capture 3", 0,
		    ARIZONA_SLIMBUS_TX_CHANNEL_ENABLE,
		    ARIZONA_SLIMTX8_ENA_SHIFT, 0,
		      arizona_slim_tx_ev,
		      SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),

SND_SOC_DAPM_AIF_IN_E("SLIMRX1", "Slimbus Playback 1", 0,
		      ARIZONA_SLIMBUS_RX_CHANNEL_ENABLE,
		      ARIZONA_SLIMRX1_ENA_SHIFT, 0,
		      arizona_slim_rx_ev,
		      SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
SND_SOC_DAPM_AIF_IN_E("SLIMRX2", "Slimbus Playback 1", 0,
		      ARIZONA_SLIMBUS_RX_CHANNEL_ENABLE,
		      ARIZONA_SLIMRX2_ENA_SHIFT, 0,
		      arizona_slim_rx_ev,
		      SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
SND_SOC_DAPM_AIF_IN_E("SLIMRX3", "Slimbus Playback 1", 0,
		      ARIZONA_SLIMBUS_RX_CHANNEL_ENABLE,
		      ARIZONA_SLIMRX3_ENA_SHIFT, 0,
		      arizona_slim_rx_ev,
		      SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
SND_SOC_DAPM_AIF_IN_E("SLIMRX4", "Slimbus Playback 1", 0,
		      ARIZONA_SLIMBUS_RX_CHANNEL_ENABLE,
		      ARIZONA_SLIMRX4_ENA_SHIFT, 0,
		      arizona_slim_rx_ev,
		      SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
SND_SOC_DAPM_AIF_IN_E("SLIMRX5", "Slimbus Playback 2", 0,
		      ARIZONA_SLIMBUS_RX_CHANNEL_ENABLE,
		      ARIZONA_SLIMRX5_ENA_SHIFT, 0,
		      arizona_slim_rx_ev,
		      SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
SND_SOC_DAPM_AIF_IN_E("SLIMRX6", "Slimbus Playback 2", 0,
		      ARIZONA_SLIMBUS_RX_CHANNEL_ENABLE,
		      ARIZONA_SLIMRX6_ENA_SHIFT, 0,
		      arizona_slim_rx_ev,
		      SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
SND_SOC_DAPM_AIF_IN_E("SLIMRX7", "Slimbus Playback 3", 0,
		      ARIZONA_SLIMBUS_RX_CHANNEL_ENABLE,
		      ARIZONA_SLIMRX7_ENA_SHIFT, 0,
		      arizona_slim_rx_ev,
		      SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
SND_SOC_DAPM_AIF_IN_E("SLIMRX8", "Slimbus Playback 3", 0,
		      ARIZONA_SLIMBUS_RX_CHANNEL_ENABLE,
...
static struct snd_soc_dai_driver wm5102_dai[] = {
	...
	{
		.name = "wm5102-slimbus-rx1",
		.id = ARIZONA_SLIMRX1,
		.playback = {
			.stream_name = "Slimbus Playback 1",
			.channels_min = 1,
			.channels_max = 4,
			.rates = WM5102_RATES,
			.formats = WM5102_FORMATS,
	...
	{
		.name = "wm5102-slimbus-rx2",
		.id = ARIZONA_SLIMRX2,
		.playback = {
			.stream_name = "Slimbus Playback 2",
			.channels_min = 1,
			.channels_max = 2,
			.rates = WM5102_RATES,
			.formats = WM5102_FORMATS,
	...
	{
		.name = "wm5102-slimbus-rx3",
		.id = ARIZONA_SLIMRX3,
		.playback = {
			.stream_name = "Slimbus Playback 3",
			.channels_min = 1,
			.channels_max = 2,
			.rates = WM5102_RATES,
			.formats = WM5102_FORMATS,
	...
	{
		.name = "wm5102-slimbus-tx1",
		.id = ARIZONA_SLIMTX1,
		.capture = {
			 .stream_name = "Slimbus Capture 1",
			 .channels_min = 1,
			 .channels_max = 4,
			 .rates = WM5102_RATES,
			 .formats = WM5102_FORMATS,
	...
	{
		.name = "wm5102-slimbus-tx2",
		.id = ARIZONA_SLIMTX2,
		.capture = {
			 .stream_name = "Slimbus Capture 2",
			 .channels_min = 1,
			 .channels_max = 2,
			 .rates = WM5102_RATES,
			 .formats = WM5102_FORMATS,
	...
	{
		.name = "wm5102-slimbus-tx3",
		.id = ARIZONA_SLIMTX3,
		.capture = {
			 .stream_name = "Slimbus Capture 3",
			 .channels_min = 1,
			 .channels_max = 2,
			 .rates = WM5102_RATES,
			 .formats = WM5102_FORMATS,
...
static int __devinit wm5102_probe(struct platform_device *pdev)
{
	...
	/* Minimum Slimbus reference clock gear by default */
