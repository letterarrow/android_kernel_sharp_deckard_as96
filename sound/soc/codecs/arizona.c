...
const char *arizona_mixer_texts[ARIZONA_NUM_MIXER_INPUTS] = {
	...
	"IN4L",
	"IN4R",
#ifdef SH_AUDIO_FBUILD /* 03-106 */
#else  /* SH_AUDIO_FBUILD */
	"AIF1RX1",
	"AIF1RX2",
	"AIF1RX3",
	"AIF1RX4",
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*03-107*/
const DECLARE_TLV_DB_SCALE(arizona_mixer_tlv, 0, 1, 0);
#else /* CONFIG_SH_AUDIO_DRIVER */ /*03-107*/
const DECLARE_TLV_DB_SCALE(arizona_mixer_tlv, -3200, 100, 0);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*03-107*/
		       ...
		       int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		ret = slim_define_ch(&arizona->slim_gd, &prop, handles, chcnt,
				     true, group);
		if (ret != 0) {
		...
		for (i = 0; i < chcnt; i++) {
			ret = slim_connect_src(&arizona->slim_gd, porth[i],
					       handles[i]);
			if (ret != 0) {
		...
		ret = slim_control_ch(&arizona->slim_gd, *group,
		    ...
		    int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		for (i = 0; i < chcnt; i++) {
			ret = slim_connect_sink(&arizona->slim_gd, &porth[i], 1,
						handles[i]);
			if (ret != 0) {
				   ...
				   unsigned int *rx_num, unsigned int *rx_slot)
{
	...
	for (i = 0; i < ARRAY_SIZE(rx_porth1); i++) {
		slim_get_slaveport(arizona->slim_gd.laddr, i, &rx_porth1[i],
				   SLIM_SINK);
	}

	for (i = 0; i < ARRAY_SIZE(rx_porth1m); i++) {
		slim_get_slaveport(arizona->slim_gd.laddr, i + 2, &rx_porth1m[i],
				   SLIM_SINK);
	}

	for (i = 0; i < ARRAY_SIZE(rx_porth2); i++) {
		slim_get_slaveport(arizona->slim_gd.laddr, i + 4, &rx_porth2[i],
				   SLIM_SINK);
	}

	for (i = 0; i < ARRAY_SIZE(rx_porth3); i++) {
		slim_get_slaveport(arizona->slim_gd.laddr, i + 6, &rx_porth3[i],
				   SLIM_SINK);
	}

	for (i = 0; i < ARRAY_SIZE(tx_porth1); i++) {
		slim_get_slaveport(arizona->slim_gd.laddr, i + 8, &tx_porth1[i],
				   SLIM_SRC);
	}

	for (i = 0; i < ARRAY_SIZE(tx_porth1s); i++) {
		slim_get_slaveport(arizona->slim_gd.laddr, i + 10, &tx_porth1s[i],
				   SLIM_SRC);
	}

	for (i = 0; i < ARRAY_SIZE(tx_porth2); i++) {
		slim_get_slaveport(arizona->slim_gd.laddr, i + 12, &tx_porth2[i],
				   SLIM_SRC);
	}

	for (i = 0; i < ARRAY_SIZE(tx_porth3); i++) {
		slim_get_slaveport(arizona->slim_gd.laddr, i + 14, &tx_porth3[i],
				   SLIM_SRC);
	}

	/* This actually allocates the channel or refcounts it if there... */
	for (i = 0; i < ARRAY_SIZE(rx_handles1); i++) {
		ret = slim_query_ch(&arizona->slim_gd, 128 + i, 
					&rx_handles1[i]);
		if (ret != 0) {
	...
	for (i = 0; i < ARRAY_SIZE(rx_handles2); i++) {
		ret = slim_query_ch(&arizona->slim_gd, 143 + i, 
					&rx_handles2[i]);
		if (ret != 0) {
			dev_err(arizona->dev, "slim_alloc_ch() failed: %d\n",
				ret);
	...
	for (i = 0; i < ARRAY_SIZE(rx_handles2); i++) {
		ret = slim_query_ch(&arizona->slim_gd, 132 + i, 
					&rx_handles3[i]);
		if (ret != 0) {
			dev_err(arizona->dev, "slim_alloc_ch() failed: %d\n",
				ret);
	...
	for (i = 0; i < ARRAY_SIZE(tx_handles1); i++) {
		ret = slim_query_ch(&arizona->slim_gd, TX_STREAM_1 + i, 
					&tx_handles1[i]);
		if (ret != 0) {
			dev_err(arizona->dev, "slim_alloc_ch() failed: %d\n",
				ret);
	...
	for (i = 0; i < ARRAY_SIZE(tx_handles2); i++) {
		ret = slim_query_ch(&arizona->slim_gd, TX_STREAM_2 + i, 
					&tx_handles2[i]);
		if (ret != 0) {
			dev_err(arizona->dev, "slim_alloc_ch() failed: %d\n",
				ret);
	...
	for (i = 0; i < ARRAY_SIZE(tx_handles3); i++) {
		ret = slim_query_ch(&arizona->slim_gd, TX_STREAM_3 + i, 
					&tx_handles3[i]);
		if (ret != 0) {
			dev_err(arizona->dev, "slim_alloc_ch() failed: %d\n",
				ret);
	...
	switch (dai->id) {
	case ARIZONA_SLIMRX1:
		*rx_num = 2;
		rx_slot[0] = 128;
		rx_slot[1] = 129;
		break;
	case ARIZONA_SLIMRX2:
		*rx_num = 1;
		rx_slot[0] = 143;
		break;
	case ARIZONA_SLIMRX3:
		*rx_num = 2;
		rx_slot[0] = 133;
		rx_slot[1] = 134;
		break;
	case ARIZONA_SLIMTX1:
		*tx_num = 2;
		tx_slot[0] = TX_STREAM_1;
		tx_slot[1] = TX_STREAM_1 + 1;
		break;
	case ARIZONA_SLIMTX2:
		*tx_num = 1;
		tx_slot[0] = TX_STREAM_2;
		break;
	case ARIZONA_SLIMTX3:
		*tx_num = 1;
...
static struct {
	...
	// gain of 16 (4) is not sufficient
	// use 256 (8)
	{ 1000000, 13500000, 8 },
#else
	{ 1000000, 13500000, 4 },
