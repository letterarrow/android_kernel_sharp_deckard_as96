...
#ifdef CONFIG_SH_AUDIO_DRIVER /*03-123*/
#define TABLA_HS_DETECT_PLUG_TIME_500MS (5 * 100)
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*03-123*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*03-008*/
...
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*03-008*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*03-121*/
	msleep(20);
#else /* CONFIG_SH_AUDIO_DRIVER */
	msleep(250);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*03-121*/
...
/* register setting */
		if (bias_mode_for_testmode) {
			if ( w->reg == TABLA_A_MICB_1_CTL)
			{
				...
				snd_soc_update_bits(codec, w->reg, 0xFF, 0xA6);
				snd_soc_update_bits(codec, TABLA_A_MICB_CFILT_2_CTL, 0xFF, 0xC0);
			}
...
/* register setting */
		if (bias_mode_for_testmode) {
			if ( w->reg == TABLA_A_MICB_1_CTL)
			{
				...
				snd_soc_update_bits(codec, TABLA_A_MICB_CFILT_2_CTL, 0xFF, 0x40);
			}
			else if ( w->reg == TABLA_A_MICB_3_CTL)
			{
				pr_debug("%s: set register\n", __func__);
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*03-010*/
	int temp;
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*03-010*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*03-010*/
		tabla_mbhc_set_rel_thres(codec, btn_high[temp]);
#else /* CONFIG_SH_AUDIO_DRIVER */
...
static bool tabla_hs_remove_settle(struct snd_soc_codec *codec)
{
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*03-123*/
	timeout = jiffies + msecs_to_jiffies(TABLA_HS_DETECT_PLUG_TIME_500MS);
#else  /* CONFIG_SH_AUDIO_DRIVER */
	timeout = jiffies + msecs_to_jiffies(TABLA_HS_DETECT_PLUG_TIME_MS);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*03-123*/
	...
	if (timedout)
		...
		pr_debug("%s: Microphone did not settle in %d seconds\n",
			 __func__, TABLA_HS_DETECT_PLUG_TIME_500MS);
#else  /* CONFIG_SH_AUDIO_DRIVER */
		pr_debug("%s: Microphone did not settle in %d seconds\n",
