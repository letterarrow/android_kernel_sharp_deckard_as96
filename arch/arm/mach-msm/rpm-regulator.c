...
enum {
	SH_MSM_RPM_VREG_DEBUG_VDD_CORE = 1U << 0,
};

static int sh_msm_rpm_vreg_debug_mask = 0;
module_param_named(
	sh_debug_mask, sh_msm_rpm_vreg_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP
);
...
int rpm_regulator_tcxo_wakelock_active(void)
{
	return wake_lock_active(&tcxo_wake_lock);
}
#endif
...
#ifdef CONFIG_SHSYS_CUST_DEBUG
			if (sh_msm_rpm_vreg_debug_mask & SH_MSM_RPM_VREG_DEBUG_VDD_CORE) {
				if ((vreg->id == vreg_id_vdd_dig) && (set != MSM_RPM_CTX_SET_0)) {
					pr_info("verg=%s voter=%d voltage=%duV\n", vreg->rdesc.name, voter, min_uV_vote[voter]);
					WARN_ON(1);
				}
			}
#endif /* CONFIG_SHSYS_CUST_DEBUG */
